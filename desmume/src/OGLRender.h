...
// The macros ENABLE_OPENGL_STANDARD and ENABLE_OPENGL_ES are used in client
// build configs. If a client wants to use OpenGL, they must declare one of
// these two macros somewhere in their build config or declare it in some
// precompiled header.

// OPENGL PLATFORM-SPECIFIC INCLUDES
#if defined(_WIN32)
#ifndef ENABLE_OPENGL_STANDARD
#define ENABLE_OPENGL_STANDARD // TODO: Declare this in the Visual Studio
                               // project file.
#endif
...
EXTERNOGLEXT(PFNGLCLEARBUFFERFVPROC,
             glClearBufferfv) // Core in v3.0 and ES v3.0
EXTERNOGLEXT(PFNGLCLEARBUFFERFIPROC,
...
// Buffer Objects
EXTERNOGLEXT(PFNGLMAPBUFFERRANGEPROC,
             glMapBufferRange) // Core in v3.0 and ES v3.0

// Vertex Array Objects
EXTERNOGLEXT(PFNGLGENVERTEXARRAYSPROC,
             glGenVertexArrays) // Core in v3.0 and ES v3.0
EXTERNOGLEXT(PFNGLDELETEVERTEXARRAYSPROC,
             glDeleteVertexArrays) // Core in v3.0 and ES v3.0
EXTERNOGLEXT(PFNGLBINDVERTEXARRAYPROC,
             glBindVertexArray) // Core in v3.0 and ES v3.0

// FBO
EXTERNOGLEXT(PFNGLGENFRAMEBUFFERSPROC,
             glGenFramebuffers) // Core in v3.0 and ES v2.0
EXTERNOGLEXT(PFNGLBINDFRAMEBUFFERPROC,
...
EXTERNOGLEXT(PFNGLDELETEFRAMEBUFFERSPROC,
             glDeleteFramebuffers) // Core in v3.0 and ES v2.0
EXTERNOGLEXT(PFNGLBLITFRAMEBUFFERPROC,
             glBlitFramebuffer) // Core in v3.0 and ES v3.0

// Multisampled FBO
EXTERNOGLEXT(PFNGLGENRENDERBUFFERSPROC,
             glGenRenderbuffers) // Core in v3.0 and ES v2.0
EXTERNOGLEXT(PFNGLBINDRENDERBUFFERPROC,
...
EXTERNOGLEXT(PFNGLBINDBUFFERBASEPROC,
...
EXTERNOGLEXT(PFNGLCLIENTWAITSYNCPROC,
...
// We need to include these explicitly for OpenGL legacy mode since the EXT
// versions of FBOs may work differently than their ARB counterparts when
...
// Most OpenGL variants don't have GL_EXT_framebuffer_object, so redeclare all
// the ARB versions to their EXT versions to avoid compile time errors in
// OGLRender.cpp.
//
// In practice, class objects for more modern variants like 3.2 Core Profile and
// ES 3.0 should override all the methods that would use FBOs so that only the
// ARB versions are actually used.
...
#define glFramebufferRenderbufferEXT(target, attachment, renderbuffertarget,   \
                                     renderbuffer)                             \
  glFramebufferRenderbuffer(target, attachment, renderbuffertarget,            \
                            renderbuffer)
#define glFramebufferTexture2DEXT(target, attachment, textarget, texture,      \
                                  level)                                       \
...
// There are some useful Apple extensions that can provide performance
// improvements or functionality in legacy OpenGL that is similar to their v3.2
// Core Profile counterparts.
...
// OpenGL ES headers before 3.2 don't have this token, so manually define it
// here.
...
// Some OpenGL variants, such as OpenGL ES, do not include certain legacy
// functions in their API. The loss of these functions will cause compile time
// errors when referenced, and so we will redeclare them to something that is
// supported for the sake of compiling.
//
// Do note that a redeclared function will most likely not work in exactly the
// same way as a native legacy version, and so implmentations are responsible
// for overriding any methods that use these legacy functions to whatever is
// available in the specific OpenGL variant.

#ifndef GL_VERSION_1_2
// These legacy functions can be promoted to later core equivalents without any
// further modification. In other words, these are one-to-one drop-in
// replacements.
typedef GLclampf GLclampd;
#define glClearDepth(depth) glClearDepthf(depth)

// 1D textures may not exist for a particular OpenGL variant, so they will be
// promoted to 2D textures instead. Implementations need to modify their GLSL
// shaders accordingly to treat any 1D textures as 2D textures instead.
#define GL_TEXTURE_1D GL_TEXTURE_2D
#define glTexImage1D(target, level, internalformat, width, border, format,     \
                     type, pixels)                                             \
  glTexImage2D(target, level, internalformat, width, 1, border, format, type,  \
               pixels)
...
// Calling glMapBuffer with an OpenGL variant that forgoes legacy functions will
// cause a GL_INVALID_OPERATION error if used in practice. Implementations need
// to override any methods that would call glMapBuffer with glMapBufferRange.
...
struct OGLRenderRef {
  ...
  CACHE_ALIGN GLushort workingCIColorBuffer16[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                              GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  CACHE_ALIGN GLuint workingCIColorBuffer32[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                            GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  CACHE_ALIGN GLuint
...
#define glDrawBuffer(theAttachment)                                            \
  glDrawBufferDESMUME((theAttachment), this->_feature.variantID)
...
protected:
  ...
  _CreateFramebufferOutput6665Program(const char *vtxShaderCString,
                                      const char *fragShaderCString);
  virtual void _DestroyFramebufferOutput6665Programs();
  virtual Render3DError
  _CreateFramebufferOutput8888Program(const char *vtxShaderCString,
                                      const char *fragShaderCString);
...
protected:
  ...
  size_t DrawPolygonsForIndexRange(const POLY *rawPolyList,
                                   const CPoly *clippedPolyList,
  ...
  virtual Render3DError CreateClearImageProgram(const char *vsCString,
                                                const char *fsCString) = 0;
  ...
  virtual Render3DError
  CreateEdgeMarkProgram(const bool isMultisample, const char *vtxShaderCString,
                        const char *fragShaderCString) = 0;
  ...
  InitPostprocessingPrograms(const char *edgeMarkVtxShader,
                             const char *edgeMarkFragShader) = 0;
...
protected:
  ...
  virtual Render3DError CreateEdgeMarkProgram(const bool isMultisample,
                                              const char *vtxShaderCString,
                                              const char *fragShaderCString);
  ...
  virtual Render3DError ZeroDstAlphaPass(
