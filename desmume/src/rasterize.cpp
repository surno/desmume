...
static FORCEINLINE void FloorDivMod(const s64 inNumerator,
                                    const s64 inDenominator, s64 &outFloor,
                                    s64 &outMod, bool &outFailure) {
...
struct edge_fx_fl {
  edge_fx_fl() {}
  edge_fx_fl(const s32 top, const s32 bottom, NDSVertex **vtx,
             SoftRasterizerPrecalculation **precalc, bool &failure);
  ...
  struct InterpolantFloat {
    ...
    FORCEINLINE void initialize(const float top, const float bottom, float dx,
                                float dy, const float inXStep,
                                const float xPrestep, const float yPrestep) {
      dx = 0.0f;
      dy *= (bottom - top);
  ...
  struct Interpolant {
    ...
    FORCEINLINE void
    initialize(const s64 top,                            // fixed
               ...
               const s64 yPrestepN, const s64 yPrestepD) // 48.16
    {
      ...
      if ((dyD != 0) && (yPrestepD != 0)) {
        const s64 dy = dyN * (bottom - top) /
                       dyD; // 16.16 * (fixed - fixed) / 16.16 = fixed

        this->curr =
            top + (yPrestepN * dy / yPrestepD) + (xPrestepN * dx / xPrestepD);
...
FORCEINLINE edge_fx_fl::edge_fx_fl(const s32 top, const s32 bottom,
                                   NDSVertex **vtx,
                                   SoftRasterizerPrecalculation **precalc,
                                   bool &failure) {
  ...
  if ((this->height != 0) || (width != 0)) {
    ...
    if (dN != 0) {
      const s64 InitialNumerator =
          (s64)(dM * 65536 * y_64 - dM * vtx[top]->position.y +
                dN * vtx[top]->position.x - 1 + dN * 65536); // 32.32
      FloorDivMod(InitialNumerator, dN * (1LL << 16), x_64, this->errorTerm,
                  failure); // 32.32, 32.32; floor is 64.0, mod is 32.32
      FloorDivMod(dM * (1LL << 16), dN * (1LL << 16), xStep_64, this->numerator,
                  failure); // 32.32, 32.32; floor is 64.0, mod is 32.32
    ...
    // Note that we can't precalculate xPrestep because x_64 can be modified by
    // an earlier call to FloorDivMod().
    const s64 xPrestepFixedN =
        (x_64 * (1LL << 16)) - (s64)vtx[top]->position.x; // 48.16 - 16.16
    ...
    const float xPrestepFloat =
        (float)((double)(x_64 * 65536 - vtx[top]->position.x) /
                65536.0); // 16.16, 16.16; result is normalized
    const float yPrestepFloat =
        precalc[top]->yPrestepNormalized; // 16.16, 16.16; result is normalized
...
#define W_PRECISION                                                            \
  (64 - W_INTEGERBITS) // We need to calculate the inverse of W, and so we want
                       // to maintain as much precision as possible for this.
#define Z_EXTRAPRECISION                                                       \
  12 // Z is calculated in 64-bit and we have the extra bits, so use them to
     // increase our precision. Mostly used for interpolating Z in
     // higher-than-native resolution rendering.
#define TC_PRECISION                                                           \
  (W_PRECISION - 4) // Texture coordinate precision is based on W_PRECISION, but
                    // needs a 16x multiplier to map correctly.

    invW.initialize(precalc[top]->invWPosition, precalc[bottom]->invWPosition,
                    dxFixedN, dxFixedD, dyFixedN, dyFixedD, this->xStep,
                    xPrestepFixedN, xPrestepFixedD, yPrestepFixedN,
                    yPrestepFixedD);

    z.initialize(precalc[top]->zPosition, precalc[bottom]->zPosition, dxFixedN,
                 dxFixedD, dyFixedN, dyFixedD, this->xStep, xPrestepFixedN,
                 xPrestepFixedD, yPrestepFixedN, yPrestepFixedD);

    s.initialize(precalc[top]->texCoord.s, precalc[bottom]->texCoord.s,
                 dxFixedN, dxFixedD, dyFixedN, dyFixedD, this->xStep,
                 xPrestepFixedN, xPrestepFixedD, yPrestepFixedN,
                 yPrestepFixedD);

    t.initialize(precalc[top]->texCoord.t, precalc[bottom]->texCoord.t,
                 dxFixedN, dxFixedD, dyFixedN, dyFixedD, this->xStep,
                 xPrestepFixedN, xPrestepFixedD, yPrestepFixedN,
                 yPrestepFixedD);

    invWFloat.initialize(precalc[top]->invWPositionNormalized,
                         precalc[bottom]->invWPositionNormalized, dxFloat,
                         dyFloat, (float)this->xStep, xPrestepFloat,
                         yPrestepFloat);

    zFloat.initialize(precalc[top]->zPositionNormalized,
                      precalc[bottom]->zPositionNormalized, dxFloat, dyFloat,
                      (float)this->xStep, xPrestepFloat, yPrestepFloat);

    sFloat.initialize(precalc[top]->texCoordNormalized.s,
                      precalc[bottom]->texCoordNormalized.s, dxFloat, dyFloat,
                      (float)this->xStep, xPrestepFloat, yPrestepFloat);

    tFloat.initialize(precalc[top]->texCoordNormalized.t,
                      precalc[bottom]->texCoordNormalized.t, dxFloat, dyFloat,
                      (float)this->xStep, xPrestepFloat, yPrestepFloat);

    for (size_t i = 0; i < 3; i++) {
      color[i].initialize(
          precalc[top]->color.component[i], precalc[bottom]->color.component[i],
          dxFixedN, dxFixedD, dyFixedN, dyFixedD, this->xStep, xPrestepFixedN,
          xPrestepFixedD, yPrestepFixedN, yPrestepFixedD);

      colorFloat[i].initialize(precalc[top]->colorNormalized.component[i],
                               precalc[bottom]->colorNormalized.component[i],
                               dxFloat, dyFloat, (float)this->xStep,
                               xPrestepFloat, yPrestepFloat);
    }
  } else {
...
static FORCEINLINE void alphaBlend(const bool isAlphaBlendingEnabled,
                                   const Color4u8 inSrc, Color4u8 &outDst) {
  if (inSrc.a == 0) {
    return;
  }
...
template <bool RENDERER>
FORCEINLINE Color4u8
RasterizerUnit<RENDERER>::_sample(const Vector2s32 &texCoord) {
  ...
  Color4u8 color;
  color.value = textureData[(sampleCoord.v
                             << this->_currentTexture->GetRenderWidthShift()) +
                            sampleCoord.u];
...
FORCEINLINE void RasterizerUnit<RENDERER>::_shade(const PolygonMode polygonMode,
                                                  const Color4u8 vtxColor,
                                                  const Vector2s32 &texCoord,
                                                  Color4u8 &outColor) {
  if (ISSHADOWPOLYGON) {
    outColor = vtxColor;
    return;
  ...
  const Color4u8 mainTexColor = (this->_currentTexture->IsSamplingEnabled())
                                    ? this->_sample(texCoord)
                                    : colorWhite;
  ...
  case POLYGON_MODE_MODULATE: {
    ...
    outColor.a = modulate_table[GFX3D_5TO6_LOOKUP(mainTexColor.a)]
                               [GFX3D_5TO6_LOOKUP(vtxColor.a)] >>
                 1;
    // dst.a = 28;
    // #ifdef _MSC_VER
  ...
  case POLYGON_MODE_TOONHIGHLIGHT: {
    const Color4u8 toonColor =
        this->_softRender->toonColor32LUT[vtxColor.r >> 1];
...
FORCEINLINE void RasterizerUnit<RENDERER>::_pixel(
    const POLYGON_ATTR polyAttr, const bool isPolyTranslucent, const u32 depth,
    const Color4u8 &vtxColor, const Vector2s32 texCoord,
    const size_t fragmentIndex, Color4u8 &dstColor) {
  const GFX3D_State &renderState = *this->_softRender->currentRenderState;
  ...
  if (polyAttr.DepthEqualTest_Enable) {
    ...
    const u32 minDepth = (u32)std::max<s32>(
        0x00000000, (s32)dstAttributeDepth - DEPTH_EQUALS_TEST_TOLERANCE);
    const u32 maxDepth = std::min<u32>(
        0x00FFFFFF, dstAttributeDepth + DEPTH_EQUALS_TEST_TOLERANCE);
  ...
  this->_shade<ISSHADOWPOLYGON>((PolygonMode)polyAttr.Mode, vtxColor, texCoord,
                                shaderOutput);

  // handle alpha test
  if (shaderOutput.a == 0 || (renderState.DISP3DCNT.EnableAlphaTest &&
  ...
  if (isOpaquePixel) {
    ...
    alphaBlend((renderState.DISP3DCNT.EnableAlphaBlending != 0), shaderOutput,
               dstColor);
...
FORCEINLINE void RasterizerUnit<RENDERER>::_drawscanline(
    const POLYGON_ATTR polyAttr, const bool isPolyTranslucent,
    Color4u8 *dstColor, const size_t framebufferWidth,
    const size_t framebufferHeight, const edge_fx_fl &pLeft,
    const edge_fx_fl &pRight) {
  ...
  if (rasterWidth == 0) {
    if (USELINEHACK) {
      ...
      rasterWidth =
          std::max(1, std::max(std::abs(leftWidth), std::abs(rightWidth)));
    } else {
      return;
    }
  }
  ...
  CACHE_ALIGN Vector2s64 texCoordInterpolated = {pLeft.s.curr, pLeft.t.curr};

  CACHE_ALIGN Color4s64 vtxColorInterpolated = {
      pLeft.color[0].curr, pLeft.color[1].curr, pLeft.color[2].curr,
      polyAttr.Alpha};

  float invWFloatInterpolated = pLeft.invWFloat.curr;
  float zFloatInterpolated = pLeft.zFloat.curr;

  CACHE_ALIGN Vector2f32 texCoordFloatInterpolated = {pLeft.sFloat.curr,
                                                      pLeft.tFloat.curr};

  CACHE_ALIGN Color4f32 vtxColorFloatInterpolated = {
      pLeft.colorFloat[0].curr, pLeft.colorFloat[1].curr,
      pLeft.colorFloat[2].curr, (float)polyAttr.Alpha / 31.0f};

  // our dx values are taken from the steps up until the right edge
  const s64 rasterWidth_dx = rasterWidth;

  const s64 invWInterpolated_dx =
      (pRight.invW.curr - invWInterpolated) / rasterWidth_dx;
  ...
  const float invWFloatInterpolated_dx =
      (pRight.invWFloat.curr - invWFloatInterpolated) / (float)rasterWidth_dx;
  const float zFloatInterpolated_dx =
      (pRight.zFloat.curr - zFloatInterpolated) / (float)rasterWidth_dx;
  ...
  const CACHE_ALIGN Color4f32 vtxColorFloatInterpolated_dx = {
      (pRight.colorFloat[0].curr - vtxColorFloatInterpolated.r) /
          (float)rasterWidth_dx,
      (pRight.colorFloat[1].curr - vtxColorFloatInterpolated.g) /
          (float)rasterWidth_dx,
      (pRight.colorFloat[2].curr - vtxColorFloatInterpolated.b) /
          (float)rasterWidth_dx,
  ...
  if ((pLeft.y < 0) || (pLeft.y >= (s32)framebufferHeight)) {
    const float gpuScalingFactorHeight =
        (float)framebufferHeight / (float)GPU_FRAMEBUFFER_NATIVE_HEIGHT;
    printf("rasterizer rendering at y=%d! oops! (x%.1f)\n", pLeft.y,
           gpuScalingFactorHeight);
    return;
  }
  ...
  if (x + rasterWidth > (s32)framebufferWidth) {
    if (RENDERER && !USELINEHACK) {
      const float gpuScalingFactorWidth =
          (float)framebufferWidth / (float)GPU_FRAMEBUFFER_NATIVE_WIDTH;
  ...
  while (rasterWidth-- > 0) {
    if (this->_softRender->currentRenderState->SWAP_BUFFERS.DepthMode) {
      // not sure about the w-buffer depth value: this value was chosen to make
      // the skybox, castle window decals, and water level render correctly in
      // SM64
      ...
      // - "Advance Wars: Days of Ruin" - Campaign map / In-game unit drawing on
      // map
      // - "Etrian Odyssey III: The Drowned City" - Main menu, when the
      // cityscape transitions between day and night
      ...
      if (this->_softRender->_enableFragmentSamplingHack) {
        depth =
            (u32)((s32)this->_round_s(zFloatInterpolated * (float)0x00FFFFFF));
      } else {
        depth = (u32)(zInterpolated / (1LL << (Z_EXTRAPRECISION + 7)));
      }

      // TODO: Hack - Drop the LSB so that the overworld map in Dragon Quest IV
      // shows up correctly.
      depth &= 0xFFFFFFFE;
    }

    if (this->_softRender->_enableFragmentSamplingHack) {
      vtxColor.r = std::max<u8>(
          0x00, (u8)std::min<float>(63.0f, vtxColorFloatInterpolated.r /
                                               invWFloatInterpolated));
      vtxColor.g = std::max<u8>(
          0x00, (u8)std::min<float>(63.0f, vtxColorFloatInterpolated.g /
                                               invWFloatInterpolated));
      vtxColor.b = std::max<u8>(
          0x00, (u8)std::min<float>(63.0f, vtxColorFloatInterpolated.b /
                                               invWFloatInterpolated));
      vtxColor.a = vtxColorInterpolated.a;

      texCoord.s = (s32)(texCoordFloatInterpolated.s * (float)texScalingFactor /
                         invWFloatInterpolated);
      texCoord.t = (s32)(texCoordFloatInterpolated.t * (float)texScalingFactor /
                         invWFloatInterpolated);
    } else {
      vtxColor.r =
          std::max<u8>(0x00, (u8)std::min<s64>(0x3F, vtxColorInterpolated.r /
                                                         invWInterpolated));
      vtxColor.g =
          std::max<u8>(0x00, (u8)std::min<s64>(0x3F, vtxColorInterpolated.g /
                                                         invWInterpolated));
      vtxColor.b =
          std::max<u8>(0x00, (u8)std::min<s64>(0x3F, vtxColorInterpolated.b /
                                                         invWInterpolated));
      vtxColor.a = vtxColorInterpolated.a;

      texCoord.s =
          (s32)(texCoordInterpolated.s * texScalingFactor / invWInterpolated);
      texCoord.t =
          (s32)(texCoordInterpolated.t * texScalingFactor / invWInterpolated);
    }

    this->_pixel<ISFRONTFACING, ISSHADOWPOLYGON>(polyAttr, isPolyTranslucent,
                                                 depth, vtxColor, texCoord, adr,
                                                 dstColor[adr]);
    adr++;
    x++;
...
void RasterizerUnit<RENDERER>::_runscanlines(
  ...
  if (USELINEHACK && (left.height == 0) && (right.height == 0) &&
      (left.y < (s32)framebufferHeight) && (left.y >= 0)) {
    ...
    if (draw) {
      this->_drawscanline<ISFRONTFACING, ISSHADOWPOLYGON, USELINEHACK>(
          polyAttr, isPolyTranslucent, dstColor, framebufferWidth,
...
#define ROTSWAP(X)                                                             \
  if (TYPE > X)                                                                \
    std::swap(this->_currentVtx[X - 1], this->_currentVtx[X]);
  ROTSWAP(1);
  ROTSWAP(2);
  ROTSWAP(3);
  ROTSWAP(4);
...
#define ROTSWAP(X)                                                             \
  if (TYPE > X)                                                                \
    std::swap(this->_currentPrecalc[X - 1], this->_currentPrecalc[X]);
  ROTSWAP(1);
  ROTSWAP(2);
  ROTSWAP(3);
  ROTSWAP(4);
...
#define CHECKY(X)                                                              \
  if (TYPE > X)                                                                \
    if (this->_currentVtx[0]->position.y > this->_currentVtx[X]->position.y)   \
      goto doswap;
    CHECKY(1);
    CHECKY(2);
    CHECKY(3);
  ...
  while (
      (this->_currentVtx[0]->position.y == this->_currentVtx[1]->position.y) &&
...
void RasterizerUnit<RENDERER>::_shape_engine(const POLYGON_ATTR polyAttr,
  ...
  for (;;) {
    ...
    if (step_left) {
      left = edge_fx_fl(_lv, lv - 1, (NDSVertex **)&this->_currentVtx,
                        (SoftRasterizerPrecalculation **)&this->_currentPrecalc,
                        failure);
    }

    if (step_right) {
      right = edge_fx_fl(
          rv, rv + 1, (NDSVertex **)&this->_currentVtx,
          (SoftRasterizerPrecalculation **)&this->_currentPrecalc, failure);
    }
    ...
    this->_runscanlines<SLI, ISFRONTFACING, ISSHADOWPOLYGON, USELINEHACK>(
        polyAttr, isPolyTranslucent, dstColor, framebufferWidth,
        framebufferHeight, isHorizontal, left, right);
...
template <bool RENDERER>
void RasterizerUnit<RENDERER>::SetSLI(const size_t startLine,
                                      const size_t endLine, bool debug) {
...
FORCEINLINE void RasterizerUnit<RENDERER>::Render() {
  ...
  const SoftRasterizerPrecalculation *softRastPrecalc =
      this->_softRender->GetPrecalculationList();
  ...
  for (size_t i = 0; i < clippedPolyCount; i++) {
    ...
    if (!clippedPoly.isPolyBackFacing) {
      if (polyAttr.Mode == POLYGON_MODE_SHADOW) {
        if (useLineHack) {
          this->_shape_engine<SLI, true, true, true>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        } else {
          this->_shape_engine<SLI, true, true, false>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        }
      } else {
        if (useLineHack) {
          this->_shape_engine<SLI, true, false, true>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        } else {
          this->_shape_engine<SLI, true, false, false>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        }
      }
      ...
      if (polyAttr.Mode == POLYGON_MODE_SHADOW) {
        if (useLineHack) {
          this->_shape_engine<SLI, false, true, true>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        } else {
          this->_shape_engine<SLI, false, true, false>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        }
      } else {
        if (useLineHack) {
          this->_shape_engine<SLI, false, false, true>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        } else {
          this->_shape_engine<SLI, false, false, false>(
              polyAttr, isPolyTranslucent, dstColor, dstWidth, dstHeight,
              (int)vertCount);
        }
      }
...
#if defined(ENABLE_AVX2)
    SoftRasterizerRenderer_AVX2 *oldRenderer =
        (SoftRasterizerRenderer_AVX2 *)CurrentRenderer;
#elif defined(ENABLE_SSE2)
    SoftRasterizerRenderer_SSE2 *oldRenderer =
        (SoftRasterizerRenderer_SSE2 *)CurrentRenderer;
#elif defined(ENABLE_NEON_A64)
    SoftRasterizerRenderer_NEON *oldRenderer =
        (SoftRasterizerRenderer_NEON *)CurrentRenderer;
#elif defined(ENABLE_ALTIVEC)
...
SoftRasterizerColorOut::SoftRasterizerColorOut(size_t w, size_t h) {
  ...
  _masterBuffer =
      malloc_alignedPage((_framebufferSize32 * 2) + (_framebufferSize16 * 2));
  memset(_masterBuffer, 0, (_framebufferSize32 * 2) + (_framebufferSize16 * 2));

  _buffer32[0] = (Color4u8 *)_masterBuffer;
  _buffer16[0] =
      (Color5551 *)((Color4u8 *)_masterBuffer + (_framebufferPixelCount * 2));
...
size_t SoftRasterizerColorOut::BindRead32() {
  ...
  if ((this->_format == NDSColorFormat_BGR888_Rev) &&
      (newReadingIdx32 != oldReadingIdx32)) {
    ColorspaceConvertBuffer6665To8888<false, false>(
        (u32 *)this->_buffer32[newReadingIdx32],
        (u32 *)this->_buffer32[newReadingIdx32], this->_framebufferPixelCount);
  }
...
Render3DError SoftRasterizerColorOut::SetSize(size_t w, size_t h) {
  ...
  this->_masterBuffer = malloc_alignedPage((this->_framebufferSize32 * 2) +
                                           (this->_framebufferSize16 * 2));
  memset(this->_masterBuffer, 0,
         (this->_framebufferSize32 * 2) + (this->_framebufferSize16 * 2));

  this->_buffer32[0] = (Color4u8 *)this->_masterBuffer;
  this->_buffer16[0] = (Color5551 *)((Color4u8 *)this->_masterBuffer +
                                     (this->_framebufferPixelCount * 2));

  this->_buffer32[1] =
      (Color4u8 *)this->_masterBuffer + this->_framebufferPixelCount;
  this->_buffer16[1] = (Color5551 *)((Color4u8 *)this->_masterBuffer +
                                     (this->_framebufferPixelCount * 2)) +
...
Render3DError SoftRasterizerColorOut::FillZero() {
  ...
  if (this->_currentReadingIdx32 != RENDER3D_RESOURCE_INDEX_NONE) {
    ...
    if (this->_buffer16[this->_currentReadingIdx16] != NULL) {
      memset(this->_buffer16[this->_currentReadingIdx16], 0,
             this->_framebufferSize16);
    }
...
Render3DError SoftRasterizerColorOut::FillColor32(const Color4u8 *src,
...
FORCEINLINE void SoftRasterizerTexture::GetRenderSamplerCoordinates(
    const u8 wrapMode, Vector2s32 &sampleCoordInOut) const {
  switch (wrapMode) {
  // flip none
  case 0x0:
    _hclamp(sampleCoordInOut.u);
    _vclamp(sampleCoordInOut.v);
    break;
  case 0x1:
    _hrepeat(sampleCoordInOut.u);
    _vclamp(sampleCoordInOut.v);
    break;
  case 0x2:
    _hclamp(sampleCoordInOut.u);
    _vrepeat(sampleCoordInOut.v);
    break;
  case 0x3:
    _hrepeat(sampleCoordInOut.u);
    _vrepeat(sampleCoordInOut.v);
...
SoftRasterizerRenderer::SoftRasterizerRenderer() {
  ...
  _clippedPolyList =
      (CPoly *)malloc_alignedCacheLine(CLIPPED_POLYLIST_SIZE * sizeof(CPoly));
  _precalc = (SoftRasterizerPrecalculation *)malloc_alignedCacheLine(
      CLIPPED_POLYLIST_SIZE * MAX_CLIPPED_VERTS *
      sizeof(SoftRasterizerPrecalculation));
  ...
  _framebufferAttributes =
      new FragmentAttributesBuffer(_framebufferWidth * _framebufferHeight);
...
void SoftRasterizerRenderer::RasterizerPrecalculate() {
  for (size_t i = 0, precalcIdx = 0; i < this->_clippedPolyCount; i++) {
    ...
    for (size_t j = 0; j < polyType; j++) {
      ...
      precalc.yPrestep =
          ((s64)y * (1LL << 16)) - (s64)vtx.position.y; // 48.16 - 16.16

      precalc.zPositionNormalized =
          (float)((double)vtx.position.z / (double)(0x7FFFFFFF));
      precalc.yPrestepNormalized = (float)((double)precalc.yPrestep / 65536.0);

      if (vtx.position.w != 0) {
        precalc.invWPosition = (1LL << W_PRECISION) / (s64)vtx.position.w;
        precalc.texCoord.s =
            (s64)vtx.texCoord.s * (1LL << TC_PRECISION) / (s64)vtx.position.w;
        precalc.texCoord.t =
            (s64)vtx.texCoord.t * (1LL << TC_PRECISION) / (s64)vtx.position.w;
        precalc.color.r =
            (s64)vtx.color.r * (1LL << W_PRECISION) / (s64)vtx.position.w;
        precalc.color.g =
            (s64)vtx.color.g * (1LL << W_PRECISION) / (s64)vtx.position.w;
        precalc.color.b =
            (s64)vtx.color.b * (1LL << W_PRECISION) / (s64)vtx.position.w;
        ...
        precalc.texCoord.s =
            (s64)vtx.texCoord.s * (1LL << TC_PRECISION) / (1LL << 12);
        precalc.texCoord.t =
            (s64)vtx.texCoord.t * (1LL << TC_PRECISION) / (1LL << 12);
...
Render3DError
SoftRasterizerRenderer::BeginRender(const GFX3D_State &renderState,
  ...
  memcpy(this->_clippedPolyList, renderGList.clippedPolyList,
  ...
  ColorspaceConvertBuffer555xTo6665Opaque<false, false, BESwapDst>(
      renderState.toonTable16, (u32 *)this->toonColor32LUT, 32);
...
void SoftRasterizerRenderer::_UpdateEdgeMarkColorTable(
  ...
  for (size_t i = 0; i < 8; i++) {
    this->_edgeMarkTable[i].value = COLOR555TO6665(
        edgeMarkColorTable[i] & 0x7FFF,
        (this->currentRenderState->DISP3DCNT.EnableAntialiasing) ? 0x10 : 0x1F);
...
void SoftRasterizerRenderer::_UpdateFogTable(const u8 *fogDensityTable) {
  const s32 fogOffset = std::min<s32>(
      std::max<s32>((s32)this->currentRenderState->fogOffset, 0), 32768);
  ...
  const s32 iMin = std::min<s32>(
      std::max<s32>(((1 + 1) << fogShiftInv) + fogOffset + 1 - fogStep, 0),
      32768);
  const s32 iMax = std::min<s32>(
      std::max<s32>(((32 + 1) << fogShiftInv) + fogOffset + 1 - fogStep, 0),
      32768);
  assert(iMin <= iMax);
  ...
  for (s32 depth = iMin; depth < iMax; depth++) {
    ...
    fogWeight = (u8)(((interp * fogDensityTable[idx - 1]) +
                      ((fogStep - interp) * fogDensityTable[idx])) >>
                     fogShiftInv);
...
Render3DError SoftRasterizerRenderer::RenderEdgeMarkingAndFog(
  ...
  for (size_t i = param.startLine * this->_framebufferWidth,
    ...
    for (size_t x = 0; x < this->_framebufferWidth; x++, i++) {
      ...
      if (param.enableFog) {
        Color4u8 fogColor;
        fogColor.value = COLOR555TO6665(param.fogColor & 0x7FFF,
                                        (param.fogColor >> 16) & 0x1F);

        const size_t fogIndex = depth >> 9;
        assert(fogIndex < 32768);
        const u8 fogWeight = (this->_framebufferAttributes->isFogged[i] != 0)
                                 ? this->_fogTable[fogIndex]
                                 : 0;

        if (!param.fogAlphaOnly) {
          dstColor.r =
              ((128 - fogWeight) * dstColor.r + fogColor.r * fogWeight) >> 7;
          dstColor.g =
              ((128 - fogWeight) * dstColor.g + fogColor.g * fogWeight) >> 7;
          dstColor.b =
              ((128 - fogWeight) * dstColor.b + fogColor.b * fogWeight) >> 7;
        }

        dstColor.a =
            ((128 - fogWeight) * dstColor.a + fogColor.a * fogWeight) >> 7;
      }
    }
  }
...
const SoftRasterizerPrecalculation *
SoftRasterizerRenderer::GetPrecalculationList() const {
...
Render3DError SoftRasterizerRenderer::ClearUsingImage(
  ...
  Color4u8 *__restrict framebufferColor =
      this->_colorOut->GetInUseFramebuffer32();
  const size_t xRatio =
  ...
  for (size_t y = 0, iw = 0; y < this->_framebufferHeight; y++) {
    ...
    for (size_t x = 0; x < this->_framebufferWidth; x++, iw++) {
      ...
      framebufferColor[iw].value = COLOR555TO6665(
  ...
  if (!this->_renderGeometryNeedsFinish) {
    if (this->_enableEdgeMark || this->_enableFog) {
      ...
      this->_threadPostprocessParam[0].fogAlphaOnly =
          (this->currentRenderState->DISP3DCNT.FogOnlyAlpha != 0);
...
#if defined(ENABLE_AVX) || defined(ENABLE_SSE2) || defined(ENABLE_NEON_A64) || \
    defined(ENABLE_ALTIVEC)
...
Render3DError SoftRasterizer_SIMD<SIMDBYTES>::SetFramebufferSize(size_t w,
  ...
  return RENDER3DERROR_NOERR;
       ...
       // defined(ENABLE_NEON_A64) || defined(ENABLE_ALTIVEC)

#if defined(ENABLE_AVX2)

void SoftRasterizerRenderer_AVX2::LoadClearValues(
    const Color4u8 &clearColor6665, const FragmentAttributes &clearAttributes) {
...
void SoftRasterizerRenderer_AVX2::ClearUsingValues_Execute(
    const size_t startPixel, const size_t endPixel) {
  Color4u8 *__restrict framebufferColor =
      this->_colorOut->GetInUseFramebuffer32();

  for (size_t i = startPixel; i < endPixel; i += sizeof(v256u8)) {
    _mm256_stream_si256((v256u32 *)(framebufferColor + i) + 0,
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(framebufferColor + i) + 1,
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(framebufferColor + i) + 2,
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(framebufferColor + i) + 3,
                        this->_clearColor_v256u32);

    _mm256_stream_si256((v256u32 *)(this->_framebufferAttributes->depth + i) +
                            0,
...
void SoftRasterizerRenderer_SSE2::ClearUsingValues_Execute(
  ...
  for (size_t i = startPixel; i < endPixel; i += sizeof(v128u8)) {
    _mm_stream_si128((v128u32 *)(framebufferColor + i) + 0,
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(framebufferColor + i) + 1,
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(framebufferColor + i) + 2,
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(framebufferColor + i) + 3,
                     this->_clearColor_v128u32);

    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i) + 0,
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i) + 1,
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i) + 2,
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i) + 3,
                     this->_clearDepth_v128u32);
...
void SoftRasterizerRenderer_NEON::LoadClearValues(
    const Color4u8 &clearColor6665, const FragmentAttributes &clearAttributes) {
  ...
  this->_clearAttrOpaquePolyID_v128u8x4.val[0] =
      vdupq_n_u8(clearAttributes.opaquePolyID);
  this->_clearAttrOpaquePolyID_v128u8x4.val[1] =
      this->_clearAttrOpaquePolyID_v128u8x4.val[0];
  this->_clearAttrOpaquePolyID_v128u8x4.val[2] =
      this->_clearAttrOpaquePolyID_v128u8x4.val[0];
  this->_clearAttrOpaquePolyID_v128u8x4.val[3] =
      this->_clearAttrOpaquePolyID_v128u8x4.val[0];

  this->_clearAttrTranslucentPolyID_v128u8x4.val[0] =
      vdupq_n_u8(clearAttributes.translucentPolyID);
  this->_clearAttrTranslucentPolyID_v128u8x4.val[1] =
      this->_clearAttrTranslucentPolyID_v128u8x4.val[0];
  this->_clearAttrTranslucentPolyID_v128u8x4.val[2] =
      this->_clearAttrTranslucentPolyID_v128u8x4.val[0];
  this->_clearAttrTranslucentPolyID_v128u8x4.val[3] =
      this->_clearAttrTranslucentPolyID_v128u8x4.val[0];

  this->_clearAttrStencil_v128u8x4.val[0] = vdupq_n_u8(clearAttributes.stencil);
  this->_clearAttrStencil_v128u8x4.val[1] =
      this->_clearAttrStencil_v128u8x4.val[0];
  this->_clearAttrStencil_v128u8x4.val[2] =
      this->_clearAttrStencil_v128u8x4.val[0];
  this->_clearAttrStencil_v128u8x4.val[3] =
      this->_clearAttrStencil_v128u8x4.val[0];

  this->_clearAttrIsFogged_v128u8x4.val[0] =
      vdupq_n_u8(clearAttributes.isFogged);
  this->_clearAttrIsFogged_v128u8x4.val[1] =
      this->_clearAttrIsFogged_v128u8x4.val[0];
  this->_clearAttrIsFogged_v128u8x4.val[2] =
      this->_clearAttrIsFogged_v128u8x4.val[0];
  this->_clearAttrIsFogged_v128u8x4.val[3] =
      this->_clearAttrIsFogged_v128u8x4.val[0];

  this->_clearAttrIsTranslucentPoly_v128u8x4.val[0] =
      vdupq_n_u8(clearAttributes.isTranslucentPoly);
  this->_clearAttrIsTranslucentPoly_v128u8x4.val[1] =
      this->_clearAttrIsTranslucentPoly_v128u8x4.val[0];
  this->_clearAttrIsTranslucentPoly_v128u8x4.val[2] =
      this->_clearAttrIsTranslucentPoly_v128u8x4.val[0];
  this->_clearAttrIsTranslucentPoly_v128u8x4.val[3] =
      this->_clearAttrIsTranslucentPoly_v128u8x4.val[0];

  this->_clearAttrPolyFacing_v128u8x4.val[0] =
      vdupq_n_u8(clearAttributes.polyFacing);
  this->_clearAttrPolyFacing_v128u8x4.val[1] =
      this->_clearAttrPolyFacing_v128u8x4.val[0];
  this->_clearAttrPolyFacing_v128u8x4.val[2] =
      this->_clearAttrPolyFacing_v128u8x4.val[0];
  this->_clearAttrPolyFacing_v128u8x4.val[3] =
      this->_clearAttrPolyFacing_v128u8x4.val[0];
}

void SoftRasterizerRenderer_NEON::ClearUsingValues_Execute(
    const size_t startPixel, const size_t endPixel) {
  Color4u8 *__restrict framebufferColor =
      this->_colorOut->GetInUseFramebuffer32();

  for (size_t i = startPixel; i < endPixel; i += (sizeof(v128u8) * 4)) {
    vst1q_u32_x4((u32 *)(framebufferColor + i) + 0,
                 this->_clearColor_v128u32x4);
    vst1q_u32_x4((u32 *)(framebufferColor + i) + 16,
                 this->_clearColor_v128u32x4);
    vst1q_u32_x4((u32 *)(framebufferColor + i) + 32,
                 this->_clearColor_v128u32x4);
    vst1q_u32_x4((u32 *)(framebufferColor + i) + 48,
                 this->_clearColor_v128u32x4);

    vst1q_u32_x4((this->_framebufferAttributes->depth + i) + 0,
                 this->_clearDepth_v128u32x4);
    vst1q_u32_x4((this->_framebufferAttributes->depth + i) + 16,
                 this->_clearDepth_v128u32x4);
    vst1q_u32_x4((this->_framebufferAttributes->depth + i) + 32,
                 this->_clearDepth_v128u32x4);
    vst1q_u32_x4((this->_framebufferAttributes->depth + i) + 48,
                 this->_clearDepth_v128u32x4);

    vst1q_u8_x4((this->_framebufferAttributes->opaquePolyID + i),
                this->_clearAttrOpaquePolyID_v128u8x4);
    vst1q_u8_x4((this->_framebufferAttributes->translucentPolyID + i),
                this->_clearAttrTranslucentPolyID_v128u8x4);
    vst1q_u8_x4((this->_framebufferAttributes->stencil + i),
                this->_clearAttrStencil_v128u8x4);
    vst1q_u8_x4((this->_framebufferAttributes->isFogged + i),
                this->_clearAttrIsFogged_v128u8x4);
    vst1q_u8_x4((this->_framebufferAttributes->isTranslucentPoly + i),
                this->_clearAttrIsTranslucentPoly_v128u8x4);
    vst1q_u8_x4((this->_framebufferAttributes->polyFacing + i),
                this->_clearAttrPolyFacing_v128u8x4);
  }
}
...
void SoftRasterizerRenderer_AltiVec::LoadClearValues(
  ...
  this->_clearAttrTranslucentPolyID_v128u8 =
      vec_splat(vec_lde(0, &translucentPolyID), 0);
  ...
  this->_clearAttrIsTranslucentPoly_v128u8 =
      vec_splat(vec_lde(0, &translucentPolyFlag), 0);
