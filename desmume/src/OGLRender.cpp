...
static const GLenum GeometryDrawBuffersEnumStandard[8][4] = {
    ...
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_FOGATTRIBUTES_ATTACHMENT_ID, GL_NONE,
     GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_POLYID_ATTACHMENT_ID, GL_NONE, GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_POLYID_ATTACHMENT_ID,
     OGL_FOGATTRIBUTES_ATTACHMENT_ID, GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_WORKING_ATTACHMENT_ID, GL_NONE, GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_WORKING_ATTACHMENT_ID,
     OGL_FOGATTRIBUTES_ATTACHMENT_ID, GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_WORKING_ATTACHMENT_ID,
     OGL_POLYID_ATTACHMENT_ID, GL_NONE},
    {OGL_COLOROUT_ATTACHMENT_ID, OGL_WORKING_ATTACHMENT_ID,
     OGL_POLYID_ATTACHMENT_ID, OGL_FOGATTRIBUTES_ATTACHMENT_ID}};

static const GLint GeometryAttachmentWorkingBufferStandard[8] = {0, 0, 0, 0,
                                                                 1, 1, 1, 1};
static const GLint GeometryAttachmentPolyIDStandard[8] = {0, 0, 1, 1,
                                                          0, 0, 2, 2};
static const GLint GeometryAttachmentFogAttributesStandard[8] = {0, 1, 0, 2,
                                                                 0, 2, 0, 3};
...
// VAO (always available in Apple's implementation of OpenGL, including old
// versions)
OGLEXT(PFNGLGENVERTEXARRAYSPROC, glGenVertexArrays) // Core in v3.0 and ES v3.0
OGLEXT(PFNGLDELETEVERTEXARRAYSPROC,
       glDeleteVertexArrays)                        // Core in v3.0 and ES v3.0
...
static void OGLLoadEntryPoints_Legacy() {
  ...
  // VAO (always available in Apple's implementation of OpenGL, including old
  // versions)
  INITOGLEXT(PFNGLGENVERTEXARRAYSPROC,
             glGenVertexArrays) // Core in v3.0 and ES v3.0
  INITOGLEXT(PFNGLDELETEVERTEXARRAYSPROC,
             glDeleteVertexArrays) // Core in v3.0 and ES v3.0
  INITOGLEXT(PFNGLBINDVERTEXARRAYPROC,
             glBindVertexArray) // Core in v3.0 and ES v3.0
...
static void OGLGetDriverVersion(const char *oglVersionString,
  ...
  // Next, check for a space that is after the dot, but before the
  // vendor-specific info (if present).
  versionStringLength =
      strlen(oglVersionString); // Set our default string length here
  const size_t endCheckLength =
      versionStringLength -
      (versionStrDot - oglVersionString); // Maximum possible length to check
  const char *checkStringLimit =
      (endCheckLength < 10)
          ? versionStrDot + endCheckLength
          : versionStrDot + 10; // But we're going to limit ourselves to
                                // checking only the first 10 characters
  ...
  // Check for any spaces before the dot. There shouldn't be any text before the
  // version number, so this step shouldn't be necessary. However, some drivers
  // can defy the OpenGL spec and include text before the version number, and so
  // we need to handle such non-compliant drivers just in case.
  ...
  char *versionSubstring =
      (char *)malloc((versionStringLength + 1) * sizeof(char));
...
Render3DError ShaderProgramCreateOGL(GLuint &vtxShaderID, GLuint &fragShaderID,
                                     GLuint &programID,
...
static void
FramebufferProcessVertexAttribEnableLEGACYOGL(const OGLFeatureInfo &feature,
  ...
  if (feature.supportVAO || feature.supportVAO_APPLE) {
    ...
    glDisableVertexAttribArray(
        OGLVertexAttributeID_Color); // Framebuffer processing doesn't use
                                     // vertex colors.
...
static void
FramebufferProcessVertexAttribDisableLEGACYOGL(const OGLFeatureInfo &feature) {
  if (!feature.supportShaders) {
    // Framebuffer processing requires shaders, so there is no legacy code path
    // for this method.
    return;
...
OpenGLRenderColorOut::OpenGLRenderColorOut(const OGLFeatureInfo &feature,
                                           size_t w, size_t h) {
  ...
  if (_willConvertColorOnGPU) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(PostprocessVtxBuffer),
                 PostprocessVtxBuffer, GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);

    if (_feature.supportVAO_APPLE ||
        (_feature.supportVAO &&
         ((_feature.variantID & OpenGLVariantFamily_Legacy) != 0))) {
    ...
    if (_feature.readPixelsBestFormat == GL_BGRA) {
      error = _CreateFramebufferOutput6665Program(
          FramebufferOutputVtxShader, FramebufferOutputBGRA6665FragShader);
      wasErrorDetected = wasErrorDetected || (error != RENDER3DERROR_NOERR);

      error = _CreateFramebufferOutput8888Program(
          FramebufferOutputVtxShader, FramebufferOutputBGRA8888FragShader);
      wasErrorDetected = wasErrorDetected || (error != RENDER3DERROR_NOERR);
    } else {
      error = _CreateFramebufferOutput6665Program(
          FramebufferOutputVtxShader, FramebufferOutputRGBA6665FragShader);
      wasErrorDetected = wasErrorDetected || (error != RENDER3DERROR_NOERR);

      // No need to create a shader program for RGBA8888 since no conversion is
    ...
    INFO("OpenGL: Cannot convert the framebuffer color on the GPU due to "
         "missing OpenGL features.\n        Will convert the framebuffer color "
         "on the CPU instead.\n");
  }

  if (_feature.supportTextureRange_APPLE &&
      _feature.supportClientStorage_APPLE) {
    // If GL_APPLE_texture_range and GL_APPLE_client_storage extensions are both
    // supported, then try using them first before any other methods. Using
    // these extensions together allows for asynchronous direct DMA transfers
    // from GPU to CPU much like PBOs, but the Apple method is slightly faster
    // and uses less memory.
    glGenTextures(2, _texColorOut);
    INFO("OpenGL: Supports asynchronous framebuffer downloads using Apple "
         "Client Storage.\n");
  } else if (_feature.supportPBO) {
    if ((_feature.variantID & OpenGLVariantFamily_ES) == 0) {
      // PBOs are supported on most platforms since PBOs only require OpenGL
      // v2.1. PBOs turn glReadPixels() from a synchronous call into an
      // asynchronous call, which helps hide copy latencies when downloading the
      // framebuffer, ultimately boosting performance.
      glGenBuffers(2, _pbo);
      INFO("OpenGL: Supports asynchronous framebuffer downloads using PBOs.\n");
    } else {
      // PBOs work by doing an asynchronous glReadPixels() call, which is very
      // beneficial to desktop-class GPUs since such devices are expected to be
      // connected to a data bus.
      //
      // However, many ARM-based mobile devices use integrated GPUs of varying
      // degrees of memory latency and implementation quality. This means that
      // the performance of an asynchronous glReadPixels() call is NOT
      // guaranteed on such devices.
      //
      // In fact, many ARM-based devices suffer devastating performance drops
      // when trying to do asynchronous framebuffer reads. Therefore, since most
      // OpenGL ES users will be running an ARM-based iGPU, we will disable PBOs
      // for OpenGL ES and stick with a traditional synchronous glReadPixels()
      // call instead.
  ...
  if (!_willConvertColorOnGPU || (_texColorOut[0] != 0) ||
      (_texColorOut[1] != 0) || (_pbo[0] == 0) || (_pbo[1] == 0)) {
  ...
  if ((_texColorOut[0] != 0) || (_texColorOut[1] != 0)) {
    glTextureRangeAPPLE(GL_TEXTURE_RECTANGLE_ARB,
                        (GLsizei)(_framebufferSize32 * 2), _masterBufferCPU32);
  ...
  if ((w != 0) && (h != 0)) {
    for (size_t i = 0; i < 2; i++) {
      if (_pbo[i] != 0) {
        ...
        glBufferData(GL_PIXEL_PACK_BUFFER, (GLsizei)_framebufferSize32, NULL,
                     GL_STREAM_READ);
        _buffer32[i] = NULL;
      }
      ...
      if (!_willConvertColorOnGPU || (_pbo[i] == 0) || (_texColorOut[i] != 0)) {
        ...
        if (_texColorOut[i] != 0) {
          ...
          glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8,
                       (GLsizei)_framebufferWidth, (GLsizei)_framebufferHeight,
                       0, _feature.readPixelsBestFormat,
                       _feature.readPixelsBestDataType, _bufferCPU32[i]);
        }
#endif
        if (_pbo[i] == 0) {
...
Render3DError OpenGLRenderColorOut::_CreateFramebufferOutput6665Program(
  ...
  if (this->_feature.supportShaderFixedLocation) {
    vsHeader << "#define IN_VTX_POSITION layout (location = "
             << OGLVertexAttributeID_Position << ") in\n";
    vsHeader << "#define IN_VTX_TEXCOORD0 layout (location = "
             << OGLVertexAttributeID_TexCoord0 << ") in\n";
    vsHeader << "#define IN_VTX_COLOR layout (location = "
             << OGLVertexAttributeID_Color << ") in\n";
  ...
  if (this->_feature.variantID & OpenGLVariantFamily_ES3) {
    ...
    fsHeader << "#define OUT_COLOR layout (location = "
             << (OGL_WORKING_ATTACHMENT_ID - GL_COLOR_ATTACHMENT0_EXT)
             << ") out\n";
  ...
  std::string vtxShaderCode =
      shaderHeader.str() + vsHeader.str() + std::string(vtxShaderCString);
  std::string fragShaderCode =
      shaderHeader.str() + fsHeader.str() + std::string(fragShaderCString);
  ...
  glBindAttribLocation(this->_pgFramebufferRGBA6665OutputID,
  ...
  const GLint uniformTexGColor = glGetUniformLocation(
      this->_pgFramebufferRGBA6665OutputID, "texInFragColor");
...
void OpenGLRenderColorOut::_DestroyFramebufferOutput6665Programs() {
  ...
  if (this->_pgFramebufferRGBA6665OutputID != 0) {
    glDetachShader(this->_pgFramebufferRGBA6665OutputID,
                   this->_vsFramebufferOutput6665ShaderID);
    glDetachShader(this->_pgFramebufferRGBA6665OutputID,
                   this->_fsFramebufferRGBA6665OutputShaderID);
...
Render3DError OpenGLRenderColorOut::_CreateFramebufferOutput8888Program(
    const char *vtxShaderCString, const char *fragShaderCString) {
  Render3DError error = OGLERROR_NOERR;
  ...
  glBindAttribLocation(this->_pgFramebufferRGBA8888OutputID,
                       OGLVertexAttributeID_Position, "inPosition");
  glBindAttribLocation(this->_pgFramebufferRGBA8888OutputID,
                       OGLVertexAttributeID_TexCoord0, "inTexCoord0");
  ...
  const GLint uniformTexGColor = glGetUniformLocation(
      this->_pgFramebufferRGBA8888OutputID, "texInFragColor");
  if (this->_feature.supportFBO) {
    glUniform1i(uniformTexGColor, OGLTextureUnitID_GColor);
  } else {
    // Reading back the output framebuffer without FBOs requires
...
void OpenGLRenderColorOut::_DestroyFramebufferOutput8888Programs() {
  ...
  if (this->_pgFramebufferRGBA8888OutputID != 0) {
    glDetachShader(this->_pgFramebufferRGBA8888OutputID,
                   this->_vsFramebufferOutput8888ShaderID);
    glDetachShader(this->_pgFramebufferRGBA8888OutputID,
                   this->_fsFramebufferRGBA8888OutputShaderID);
...
Render3DError OpenGLRenderColorOut::_FramebufferConvertColorFormat() {
  if (this->_willConvertColorOnGPU &&
      ((this->_format == NDSColorFormat_BGR666_Rev) ||
       (this->_feature.readPixelsBestFormat == GL_BGRA))) {
    const GLuint convertProgramID = (this->_format == NDSColorFormat_BGR666_Rev)
                                        ? this->_pgFramebufferRGBA6665OutputID
                                        : this->_pgFramebufferRGBA8888OutputID;
    ...
    if (this->_feature.supportFBO) {
      ...
      glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0,
                          (GLsizei)this->_framebufferWidth,
                          (GLsizei)this->_framebufferHeight);
      glActiveTexture(GL_TEXTURE0);
    }

    glViewport(0, 0, (GLsizei)this->_framebufferWidth,
               (GLsizei)this->_framebufferHeight);
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_STENCIL_TEST);
    glDisable(GL_BLEND);

    FramebufferProcessVertexAttribEnableLEGACYOGL(this->_feature,
                                                  this->_vaoPostprocessStatesID,
                                                  this->_vboPostprocessVtxID);
...
void OpenGLRenderColorOut::Reset() { this->FillZero(); }
...
size_t OpenGLRenderColorOut::BindRead32() {
  ...
  if ((oldReadingIdx32 != RENDER3D_RESOURCE_INDEX_NONE) &&
      (this->_currentReadingIdx32 != oldReadingIdx32)) {
  ...
  if (this->_pbo[this->_currentReadingIdx32] != 0) {
    if (this->_buffer32[this->_currentReadingIdx32] == NULL) {
      glBindBuffer(GL_PIXEL_PACK_BUFFER,
                   this->_pbo[this->_currentReadingIdx32]);
    ...
    if (this->_texColorOut[this->_currentReadingIdx32] != 0) {
      glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
                    this->_texColorOut[this->_currentReadingIdx32]);
      glGetTexImage(GL_TEXTURE_RECTANGLE_ARB, 0,
                    this->_feature.readPixelsBestFormat,
                    this->_feature.readPixelsBestDataType,
                    this->_bufferCPU32[this->_currentReadingIdx32]);
    } else
#endif
    {
      glReadPixels(0, 0, (GLsizei)this->_framebufferWidth,
                   ...
                   this->_feature.readPixelsBestFormat,
                   this->_feature.readPixelsBestDataType,
                   this->_buffer32[this->_currentReadingIdx32]);
    }
  }
    ...
    if (this->_format == NDSColorFormat_BGR666_Rev) {
      if (this->_feature.readPixelsBestFormat == GL_BGRA) {
        ColorspaceConvertBuffer8888To6665<true, false>(
            (u32 *)this->_buffer32[this->_currentReadingIdx32],
            (u32 *)this->_bufferCPU32[this->_currentReadingIdx32],
            this->_framebufferPixelCount);
      } else {
        ColorspaceConvertBuffer8888To6665<false, false>(
            ...
            this->_framebufferPixelCount);
               ...
               (this->_feature.readPixelsBestFormat == GL_BGRA)) {
      ColorspaceCopyBuffer32<true, false>(
          (u32 *)this->_buffer32[this->_currentReadingIdx32],
          (u32 *)this->_bufferCPU32[this->_currentReadingIdx32],
          this->_framebufferPixelCount);
    }
...
void OpenGLRenderColorOut::UnbindRenderer(const size_t idxRead) {
  ...
  if (this->_pbo[idxRead] != 0) {
    ...
    glReadPixels(0, 0, (GLsizei)this->_framebufferWidth,
                 (GLsizei)this->_framebufferHeight,
                 this->_feature.readPixelsBestFormat,
                 this->_feature.readPixelsBestDataType, 0);
  }
#if defined(GL_APPLE_texture_range) && defined(GL_APPLE_client_storage)
    ...
    glCopyTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 0, 0,
                        (GLsizei)this->_framebufferWidth,
                        (GLsizei)this->_framebufferHeight);
    glFlush(); // glCopyTexSubImage2D() doesn't do an implicit flush like
               // glReadPixels() does, so we need to explicitly call glFlush()
               // here.
  }
#endif
}
...
Render3DError OpenGLRenderColorOut::SetSize(size_t w, size_t h) {
  ...
  if (oldMasterBuffer32 != NULL) {
    newMasterBuffer32 =
        (Color4u8 *)malloc_alignedPage(this->_framebufferSize32 * 2);
#if defined(GL_APPLE_texture_range) && defined(GL_APPLE_client_storage)
    if ((this->_texColorOut[0] != 0) || (this->_texColorOut[1] != 0)) {
      glTextureRangeAPPLE(GL_TEXTURE_RECTANGLE_ARB,
                          (GLsizei)(this->_framebufferSize32 * 2),
                          newMasterBuffer32);
      glPixelStorei(GL_UNPACK_CLIENT_STORAGE_APPLE, GL_TRUE);
    }
#endif
  ...
  for (size_t i = 0; i < 2; i++) {
    if (this->_pbo[i] != 0) {
      ...
      glBufferData(GL_PIXEL_PACK_BUFFER, this->_framebufferSize32, NULL,
                   GL_STREAM_READ);
    ...
    if (this->_bufferCPU32[i] != NULL) {
      this->_bufferCPU32[i] =
          newMasterBuffer32 + (this->_framebufferPixelCount * i);
#if defined(GL_APPLE_texture_range) && defined(GL_APPLE_client_storage)
      if (this->_texColorOut[i] != 0) {
        ...
        glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_STORAGE_HINT_APPLE,
                        GL_STORAGE_SHARED_APPLE);
        glTexImage2D(
            GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8,
            (GLsizei)this->_framebufferWidth, (GLsizei)this->_framebufferHeight,
            0, this->_feature.readPixelsBestFormat,
            this->_feature.readPixelsBestDataType, this->_bufferCPU32[i]);
      }
#endif
      if (this->_pbo[i] == 0) {
    ...
    free_aligned(this->_buffer16[i]);
    this->_buffer16[i] =
        (Color5551 *)malloc_alignedPage(this->_framebufferSize16);
  }
  ...
  if (this->_feature.supportShaders) {
    ...
    if (this->_feature.readPixelsBestFormat == GL_BGRA) {
      this->_CreateFramebufferOutput6665Program(
          FramebufferOutputVtxShader, FramebufferOutputBGRA6665FragShader);
      this->_CreateFramebufferOutput8888Program(
          FramebufferOutputVtxShader, FramebufferOutputBGRA8888FragShader);
    } else {
      this->_CreateFramebufferOutput6665Program(
          FramebufferOutputVtxShader, FramebufferOutputRGBA6665FragShader);
...
Render3DError OpenGLRenderColorOut::FillZero() {
  ...
  if ((this->_pbo[bufferIdx32] != 0) && this->_willConvertColorOnGPU &&
      (bufferIdx32 == this->_currentReadingIdx32)) {
...
Render3DError OpenGLRenderColorOut::FillColor32(const Color4u8 *src,
                                                const bool isSrcNativeSize) {
  // TODO: Implement this method.
  // Not a high priority, since the only function that calls this is
  // gfx3d_FinishLoadStateBufferRead(), which historically was never able to
  // copy the saved framebuffer into OpenGL in the first place.
  return RENDER3DERROR_NOERR;
}
...
void OpenGLRenderColorOut::SetFBORenderID(GLuint i) { this->_fboRenderID = i; }
  ...
  case 1: {
    ...
    if (forceTextureInit || !this->_isTexInited) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->_sizeS, this->_sizeT, 0,
                   GL_RGBA, GL_UNSIGNED_BYTE, textureSrc);
    } else {
  ...
  case 2: {
    ...
    if (forceTextureInit || !this->_isTexInited) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->_sizeS * 2,
                   this->_sizeT * 2, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                   this->_upscaleBuffer);
      glTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, this->_sizeS * 1,
                   this->_sizeT * 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureSrc);
    } else {
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, this->_sizeS * 2,
                      this->_sizeT * 2, GL_RGBA, GL_UNSIGNED_BYTE,
                      this->_upscaleBuffer);
      glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0, this->_sizeS * 1,
                      this->_sizeT * 1, GL_RGBA, GL_UNSIGNED_BYTE, textureSrc);
    }
    break;
  }
  ...
  case 4: {
    ...
    if (forceTextureInit || !this->_isTexInited) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->_sizeS * 4,
                   this->_sizeT * 4, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                   this->_upscaleBuffer);
      ...
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, this->_sizeS * 4,
                      this->_sizeT * 4, GL_RGBA, GL_UNSIGNED_BYTE,
                      this->_upscaleBuffer);
...
template <OpenGLVariantID VARIANTID> static Render3D *OpenGLRendererCreate() {
  ...
  case OpenGLVariantID_Legacy_1_2:
    strncpy(variantStringFull, "OpenGL 1.2 (forced)",
            sizeof(variantStringFull));
    strncpy(variantFamilyString, "OpenGL", sizeof(variantFamilyString));
    break;

  case OpenGLVariantID_Legacy_2_0:
    strncpy(variantStringFull, "OpenGL 2.0 (forced)",
            sizeof(variantStringFull));
    strncpy(variantFamilyString, "OpenGL", sizeof(variantFamilyString));
    break;

  case OpenGLVariantID_Legacy_2_1:
    strncpy(variantStringFull, "OpenGL 2.1 (forced)",
            sizeof(variantStringFull));
    strncpy(variantFamilyString, "OpenGL", sizeof(variantFamilyString));
    break;

  case OpenGLVariantID_CoreProfile_3_2:
    strncpy(variantStringFull, "OpenGL 3.2 (forced)",
            sizeof(variantStringFull));
    strncpy(variantFamilyString, "OpenGL", sizeof(variantFamilyString));
    break;
  ...
  if (oglrender_init == NULL) {
    INFO("%s: oglrender_init is unassigned. Clients must assign this function "
         "pointer and have a working context before running OpenGL.\n",
         variantStringFull);
    return newRenderer;
  }

  if (oglrender_beginOpenGL == NULL) {
    INFO("%s: oglrender_beginOpenGL is unassigned. Clients must assign this "
         "function pointer before running OpenGL.\n",
         variantStringFull);
    return newRenderer;
  }

  if (oglrender_endOpenGL == NULL) {
    INFO("%s: oglrender_endOpenGL is unassigned. Clients must assign this "
         "function pointer before running OpenGL.\n",
         variantStringFull);
    return newRenderer;
  }
  ...
  if (VARIANTID & OpenGLVariantFamily_Standard) {
    if (!IsOpenGLDriverVersionSupported(
      ...
      INFO("%s: This context does not support OpenGL v%u.%u.%u or later. "
           "Disabling 3D renderer.\n[ Context Info -\n    Version: %s\n    "
           "Vendor: %s\n    Renderer: %s ]\n",
           variantStringFull,
           OGLRENDER_STANDARD_MINIMUM_CONTEXT_VERSION_REQUIRED_MAJOR,
           OGLRENDER_STANDARD_MINIMUM_CONTEXT_VERSION_REQUIRED_MINOR,
    ...
    if (!IsOpenGLDriverVersionSupported(
      ...
      INFO("%s: This context does not support OpenGL ES v%u.%u.%u or later. "
           "Disabling 3D renderer.\n[ Context Info -\n    Version: %s\n    "
           "Vendor: %s\n    Renderer: %s ]\n",
           variantStringFull,
           OGLRENDER_ES_MINIMUM_CONTEXT_VERSION_REQUIRED_MAJOR,
    ...
    INFO("%s: This renderer does not support the OpenGL variant requested by "
         "this context. Disabling 3D renderer.\n[ Context Info -\n    Version: "
         "%s\n    Vendor: %s\n    Renderer: %s ]\n",
         variantStringFull, oglVersionString, oglVendorString,
         oglRendererString);
    ENDGL();
    return newRenderer;
  }
  ...
  if (VARIANTID == OpenGLVariantID_ES3_3_0) {
    if ((OGLLoadEntryPoints_ES_3_0_Func != NULL) &&
        (OGLCreateRenderer_ES_3_0_Func != NULL)) {
    ...
    if ((VARIANTID == OpenGLVariantID_StandardAuto) ||
        (VARIANTID == OpenGLVariantID_CoreProfile_3_2)) {
    ...
    if ((VARIANTID == OpenGLVariantID_StandardAuto) ||
        (VARIANTID == OpenGLVariantID_LegacyAuto)) {
      // If the renderer doesn't initialize with OpenGL v3.2 or higher, fall
      // back to one of the lower versions.
      if (newRenderer == NULL) {
        OGLLoadEntryPoints_Legacy();
        if (IsOpenGLDriverVersionSupported(2, 1, 0)) {
          ...
          newRenderer = new OpenGLRenderer_1_2;
               ...
               IsOpenGLDriverVersionSupported(2, 1, 0)) {
      OGLLoadEntryPoints_Legacy();
      newRenderer = new OpenGLRenderer_2_1;
    } else if ((VARIANTID == OpenGLVariantID_Legacy_2_0) &&
               IsOpenGLDriverVersionSupported(2, 0, 0)) {
      OGLLoadEntryPoints_Legacy();
      newRenderer = new OpenGLRenderer_2_0;
    } else if ((VARIANTID == OpenGLVariantID_Legacy_1_2) &&
               IsOpenGLDriverVersionSupported(1, 2, 0)) {
      OGLLoadEntryPoints_Legacy();
      newRenderer = new OpenGLRenderer_1_2;
    }
  }
  ...
  if (error != OGLERROR_NOERR) {
    if (error == OGLERROR_DRIVER_VERSION_TOO_OLD) {
      INFO("%s: This driver does not support the minimum feature set required "
           "to run this renderer. Disabling 3D renderer.\n[ Context Info -\n   "
           " Version: %s\n    Vendor: %s\n    Renderer: %s ]\n",
           variantFamilyString, oglVersionString, oglVendorString,
           oglRendererString);
    } else if (newRenderer->IsVersionSupported(1, 5, 0) &&
               (error == OGLERROR_VBO_UNSUPPORTED)) {
      INFO("%s: VBOs are not available, even though this version of OpenGL "
      ...
      INFO("%s: Shaders are not working, even though they should be on this "
           "version of OpenGL. Disabling 3D renderer.\n[ Context Info -\n    "
           "Version: %s\n    Vendor: %s\n    Renderer: %s ]\n",
           variantFamilyString, oglVersionString, oglVendorString,
           oglRendererString);
    } else if (newRenderer->IsVersionSupported(2, 1, 0) &&
               (error == OGLERROR_PBO_UNSUPPORTED)) {
      INFO("%s: PBOs are not available, even though this version of OpenGL "
           "requires them. Disabling 3D renderer.\n[ Context Info -\n    "
           "Version: %s\n    Vendor: %s\n    Renderer: %s ]\n",
           variantFamilyString, oglVersionString, oglVendorString,
           oglRendererString);
               ...
               ((OGLLoadEntryPoints_3_2_Func != NULL) ||
                (OGLLoadEntryPoints_ES_3_0_Func != NULL))) {
      INFO("%s: FBOs are not available, even though this version of OpenGL "
           "requires them. Disabling 3D renderer.\n[ Context Info -\n    "
           "Version: %s\n    Vendor: %s\n    Renderer: %s ]\n",
           variantFamilyString, oglVersionString, oglVendorString,
           oglRendererString);
  ...
  INFO("%s: Renderer initialized successfully (v%i.%i.%i).\n[ Context Info -\n "
       "   Version: %s\n    Vendor: %s\n    Renderer: %s ]\n",
       variantFamilyString, vMajor, vMinor, vRevision, oglVersionString,
...
OpenGLRenderer::OpenGLRenderer() {
  ...
  _workingTextureUnpackBuffer =
      (Color4u8 *)malloc_alignedPage(1024 * 1024 * sizeof(Color4u8));
...
bool OpenGLRenderer::IsVersionSupported(
  ...
  if ((vMajor > checkVersionMajor) ||
      ...
      (vMajor >= checkVersionMajor && vMinor >= checkVersionMinor &&
       vRevision >= checkVersionRevision)) {
    result = true;
  }
...
size_t OpenGLRenderer::DrawPolygonsForIndexRange(
    const POLY *rawPolyList, const CPoly *clippedPolyList,
    const size_t clippedPolyCount, size_t firstIndex, size_t lastIndex,
    size_t &indexOffset, POLYGON_ATTR &lastPolyAttr) {
  ...
  GLushort *indexBufferPtr = (this->_feature.supportVBO)
  ...
  for (size_t i = firstIndex; i <= lastIndex; i++) {
    ...
    if (lastPolyAttr.value != rawPoly.attribute.value) {
      ...
      this->SetupPolygon(rawPoly, (DRAWMODE != OGLPolyDrawMode_DrawOpaquePolys),
                         (DRAWMODE != OGLPolyDrawMode_ZeroAlphaPass),
                         clippedPoly.isPolyBackFacing);
    }

    // Set up the texture if it changed
    if (lastTexParams.value != rawPoly.texParam.value ||
    ...
    // extra diagonal line.
    const size_t LUTIndex = (!GFX3D_IsPolyWireframe(rawPoly))
                                ? rawPoly.vtxFormat
                                : (0x08 | rawPoly.vtxFormat);
    ...
    } else if ((rawPoly.texParam.PackedFormat == TEXMODE_A3I5) ||
               (rawPoly.texParam.PackedFormat == TEXMODE_A5I3)) {
      this->DrawAlphaTexturePolygon<DRAWMODE>(
          polyPrimitive, vertIndexCount, indexBufferPtr,
          rawPoly.attribute.DepthEqualTest_Enable,
          rawPoly.attribute.TranslucentDepthWrite_Enable,
          GFX3D_IsPolyWireframe(rawPoly) || GFX3D_IsPolyOpaque(rawPoly),
          rawPoly.attribute.PolygonID, !clippedPoly.isPolyBackFacing);
    } else {
  ...
  if (this->_feature.supportShaders) {
    if ((DRAWMODE != OGLPolyDrawMode_ZeroAlphaPass) && performDepthEqualTest &&
      ...
      if (DRAWMODE == OGLPolyDrawMode_DrawTranslucentPolys) {
        ...
        glUniform1f(
            OGLRef.uniformPolyDepthOffset[this->_geometryProgramFlags.value],
            (float)DEPTH_EQUALS_TEST_TOLERANCE / 16777215.0f);
      ...
      if (this->_emulateDepthLEqualPolygonFacing &&
          this->_isDepthLEqualPolygonFacingSupported && isPolyFrontFacing) {
    ...
    } else // Draw the polygon as completely opaque.
    {
      ...
      if (this->_emulateDepthLEqualPolygonFacing &&
          this->_isDepthLEqualPolygonFacingSupported) {
...
Render3DError OpenGLRenderer::DrawOtherPolygon(const GLenum polyPrimitive,
  ...
  if ((DRAWMODE != OGLPolyDrawMode_ZeroAlphaPass) && performDepthEqualTest &&
      this->_emulateNDSDepthCalculation && this->_feature.supportShaders) {
    glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
    glDepthMask(GL_FALSE);
    ...
    glUniform1f(
        OGLRef.uniformPolyDepthOffset[this->_geometryProgramFlags.value], 0.0f);
    glDepthFunc(GL_ALWAYS);
...
OpenGLRenderer::ApplyRenderingSettings(const GFX3D_State &renderState) {
  ...
  this->_enableMultisampledRendering = ((this->_selectedMultisampleSize >= 2) &&
                                        this->_feature.supportMultisampledFBO);
  this->_willUseMultisampleShaders =
      this->_feature.supportSampleShading && this->_enableMultisampledRendering;
  if (this->_willUseMultisampleShaders != oldMultisampleShadingFlag) {
    // Fog program IDs don't have their own multisampled versions of the IDs,
    // and so we need to reset all of the existing IDs so that the fog programs
    // can be regenerated
...
Render3DError OpenGLRenderer_1_2::InitExtensions() {
  ...
  this->_feature.supportTextureRange_APPLE =
      this->IsExtensionPresent(&oglExtensionSet, "GL_APPLE_texture_range");
  this->_feature.supportClientStorage_APPLE =
      this->IsExtensionPresent(&oglExtensionSet, "GL_APPLE_client_storage");

  // Mirrored Repeat Mode Support
  this->_feature.supportTextureMirroredRepeat =
      this->IsVersionSupported(1, 4, 0) ||
      this->IsExtensionPresent(&oglExtensionSet,
                               "GL_ARB_texture_mirrored_repeat");
  this->_feature.stateTexMirroredRepeat =
      (this->_feature.supportTextureMirroredRepeat) ? GL_MIRRORED_REPEAT
                                                    : GL_REPEAT;

  // Blending Support
  this->_feature.supportBlendFuncSeparate =
      this->IsVersionSupported(1, 4, 0) ||
      this->IsExtensionPresent(&oglExtensionSet, "GL_EXT_blend_func_separate");
  this->_feature.supportBlendEquationSeparate =
      this->IsVersionSupported(2, 0, 0) ||
      this->IsExtensionPresent(&oglExtensionSet,
                               "GL_EXT_blend_equation_separate");

  // Don't use ARB versions since we're using the EXT versions for backwards
  // compatibility.
  this->_feature.supportFBO =
  ...
  // The internal format of FBOs is GL_RGBA, so we will match that format for
  // glReadPixels. But the traditional format before FBOs was GL_BGRA, which is
  // also the fastest format for glReadPixels when using the legacy default
  // framebuffer.
  this->_feature.readPixelsBestFormat =
      (this->_feature.supportFBO) ? GL_RGBA : GL_BGRA;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, (GLsizei)this->_framebufferWidth,
               (GLsizei)this->_framebufferHeight, 0,
               this->_feature.readPixelsBestFormat,
               this->_feature.readPixelsBestDataType, NULL);
  glActiveTexture(GL_TEXTURE0);

  this->_feature.supportVBO =
  ...
  this->_feature.supportFBOBlit =
      this->_feature.supportFBO &&
      this->IsExtensionPresent(&oglExtensionSet, "GL_EXT_framebuffer_blit");
  if (!this->_feature.supportFBOBlit) {
    INFO("OpenGL: FBO blitting is unsupported. Some emulation features will be "
         "disabled.\n");
  }
  ...
  if (!this->_feature.supportShaders) {
    ...
    // If shaders aren't available, we need to take care of the vertex data
    // format conversion on the CPU instead of on the GPU using these
    // client-side buffers.
    OGLRef.position4fBuffer =
        (GLfloat *)malloc_alignedPage(VERTLIST_SIZE * sizeof(GLfloat) * 4);
    OGLRef.texCoord2fBuffer =
        (GLfloat *)malloc_alignedPage(VERTLIST_SIZE * sizeof(GLfloat) * 2);
    OGLRef.color4fBuffer =
        (GLfloat *)malloc_alignedPage(VERTLIST_SIZE * sizeof(GLfloat) * 4);
  }

  this->_feature.supportVAO =
      this->_feature.supportVBO &&
      this->IsExtensionPresent(&oglExtensionSet, "GL_ARB_vertex_array_object");
  this->_feature.supportVAO_APPLE =
      this->_feature.supportVBO &&
      this->IsExtensionPresent(&oglExtensionSet,
  ...
  this->_deviceInfo.isEdgeMarkSupported = this->_feature.supportShaders &&
                                          this->_feature.supportVBO &&
                                          this->_feature.supportFBO;
  this->_deviceInfo.isFogSupported = this->_feature.supportShaders &&
                                     this->_feature.supportVBO &&
                                     this->_feature.supportFBO;
  this->_deviceInfo.isTextureSmoothingSupported = this->_feature.supportShaders;
  this->_isDepthLEqualPolygonFacingSupported = this->_feature.supportShaders &&
                                               this->_feature.supportVBO &&
                                               this->_feature.supportFBO;
  ...
  this->_colorOut = new OpenGLRenderColorOut(
      this->_feature, this->_framebufferWidth, this->_framebufferHeight);
...
Render3DError OpenGLRenderer_1_2::CreateVBOs() {
  ...
  glBufferData(GL_ARRAY_BUFFER, VERTLIST_SIZE * sizeof(NDSVertex), NULL,
               GL_STREAM_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, OGLRef.iboGeometryIndexID);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(OGLRef.vertIndexBuffer), NULL,
               GL_STREAM_DRAW);
...
Render3DError OpenGLRenderer_1_2::CreateVAOs() {
  ...
  if (this->_feature.supportShaders) {
    ...
    glVertexAttribPointer(OGLVertexAttributeID_Position, 4, GL_INT, GL_FALSE,
                          sizeof(NDSVertex),
                          (const GLvoid *)offsetof(NDSVertex, position));
    glVertexAttribPointer(OGLVertexAttributeID_TexCoord0, 2, GL_INT, GL_FALSE,
                          sizeof(NDSVertex),
                          (const GLvoid *)offsetof(NDSVertex, texCoord));
    glVertexAttribPointer(OGLVertexAttributeID_Color, 4, GL_UNSIGNED_BYTE,
                          GL_FALSE, sizeof(NDSVertex),
                          (const GLvoid *)offsetof(NDSVertex, color));
  }
#if defined(GL_VERSION_1_2)
  else {
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
Render3DError OpenGLRenderer_1_2::CreateFBOs() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8_EXT,
               (GLsizei)this->_framebufferWidth,
               (GLsizei)this->_framebufferHeight, 0, GL_DEPTH_STENCIL_EXT,
  ...
  GLint *tempClearImageBuffer = (GLint *)calloc(
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, GPU_FRAMEBUFFER_NATIVE_WIDTH,
               GPU_FRAMEBUFFER_NATIVE_HEIGHT, 0, GL_RGBA,
               OGLRef.textureSrcTypeCIColor, tempClearImageBuffer);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, GPU_FRAMEBUFFER_NATIVE_WIDTH,
               GPU_FRAMEBUFFER_NATIVE_HEIGHT, 0, GL_RGBA,
               OGLRef.textureSrcTypeCIFog, tempClearImageBuffer);
  ...
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, OGL_CI_COLOROUT_ATTACHMENT_ID,
                            GL_TEXTURE_2D, OGLRef.texCIColorID, 0);
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                            OGL_CI_FOGATTRIBUTES_ATTACHMENT_ID, GL_TEXTURE_2D,
  ...
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, OGL_COLOROUT_ATTACHMENT_ID,
                            GL_TEXTURE_2D, OGLRef.texGColorID, 0);
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, OGL_POLYID_ATTACHMENT_ID,
                            GL_TEXTURE_2D, OGLRef.texGPolyID, 0);
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, OGL_FOGATTRIBUTES_ATTACHMENT_ID,
                            GL_TEXTURE_2D, OGLRef.texGFogAttrID, 0);
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, OGL_WORKING_ATTACHMENT_ID,
...
Render3DError OpenGLRenderer_1_2::CreateMultisampledFBO(GLsizei numSamples) {
  ...
  glRenderbufferStorageMultisampleEXT(GL_RENDERBUFFER_EXT, numSamples, GL_RGBA8,
                                      (GLsizei)this->_framebufferWidth,
                                      (GLsizei)this->_framebufferHeight);
  ...
  glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, OGL_COLOROUT_ATTACHMENT_ID,
                               GL_RENDERBUFFER_EXT, OGLRef.rboMSGColorID);
  glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, OGL_POLYID_ATTACHMENT_ID,
  ...
  glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, OGL_WORKING_ATTACHMENT_ID,
...
void OpenGLRenderer_1_2::ResizeMultisampledFBOs(GLsizei numSamples) {
  ...
  if (!this->_feature.supportMultisampledFBO || (numSamples == 1) ||
...
Render3DError OpenGLRenderer_1_2::CreateGeometryPrograms() {
  ...
  glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, 32, 0, GL_RGBA,
               OGLRef.textureSrcTypeToonTable, NULL);
  ...
  glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, 8, 0, GL_RGBA,
               OGLRef.textureSrcTypeEdgeColor, NULL);
  ...
  glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexImage1D(GL_TEXTURE_1D, 0, GL_LUMINANCE, 32, 0, GL_RED, GL_UNSIGNED_BYTE,
               NULL);
  glActiveTexture(GL_TEXTURE0);
  ...
  vtxShaderHeader << "#define DEPTH_EQUALS_TEST_TOLERANCE "
                  << DEPTH_EQUALS_TEST_TOLERANCE << ".0\n";
  vtxShaderHeader << "\n";

  std::string vtxShaderCode =
      vtxShaderHeader.str() + std::string(GeometryVtxShader_100);
  ...
  fragShaderHeader << "#define FRAMEBUFFER_SIZE_Y " << this->_framebufferHeight
                   << ".0 \n";
  fragShaderHeader << "\n";
  fragShaderHeader << "#define OUTFRAGCOLOR "
                   << ((this->_feature.supportFBO) ? "gl_FragData[0]"
                                                   : "gl_FragColor")
                   << "\n";
  fragShaderHeader << "\n";

  for (size_t flagsValue = 0; flagsValue < 128;
    ...
    shaderFlags << "#define ENABLE_FOG "
                << ((programFlags.EnableFog && this->_feature.supportVBO &&
                     this->_feature.supportFBO)
                        ? 1
                        : 0)
                << "\n"; // Do not rely on this->_deviceInfo.isFogSupported
                         // because it hasn't been set yet.
    shaderFlags << "#define ENABLE_EDGE_MARK "
                << ((programFlags.EnableEdgeMark && this->_feature.supportVBO &&
                     this->_feature.supportFBO)
                        ? 1
                        : 0)
                << "\n"; // Do not rely on this->_deviceInfo.isEdgeMarkSupported
                         // because it hasn't been set yet.
    shaderFlags << "#define DRAW_MODE_OPAQUE "
                << ((programFlags.OpaqueDrawMode && this->_feature.supportVBO &&
                     this->_feature.supportFBO)
                        ? 1
                        : 0)
                << "\n";
    shaderFlags << "\n";
    shaderFlags
        << "#define ATTACHMENT_WORKING_BUFFER "
        << this->_geometryAttachmentWorkingBuffer[programFlags.DrawBuffersMode]
        << "\n";
    shaderFlags << "#define ATTACHMENT_POLY_ID "
                << this->_geometryAttachmentPolyID[programFlags.DrawBuffersMode]
                << "\n";
    shaderFlags
        << "#define ATTACHMENT_FOG_ATTRIBUTES "
        << this->_geometryAttachmentFogAttributes[programFlags.DrawBuffersMode]
        << "\n";
    shaderFlags << "\n";
    ...
    error =
        ShaderProgramCreateOGL(OGLRef.vertexGeometryShaderID,
    ...
    const GLint uniformTexToonTable = glGetUniformLocation(
        OGLRef.programGeometryID[flagsValue], "texToonTable");
    ...
    if (this->_emulateDepthLEqualPolygonFacing &&
      ...
      const GLint uniformTexBackfacing = glGetUniformLocation(
          OGLRef.programGeometryID[flagsValue], "inDstBackFacing");
    ...
    OGLRef.uniformPolyIsBackFacing[flagsValue] = glGetUniformLocation(
        OGLRef.programGeometryID[flagsValue], "polyIsBackFacing");
    OGLRef.uniformPolyDrawShadow[flagsValue] = glGetUniformLocation(
        OGLRef.programGeometryID[flagsValue], "polyDrawShadow");
    OGLRef.uniformPolyDepthOffset[flagsValue] = glGetUniformLocation(
        OGLRef.programGeometryID[flagsValue], "polyDepthOffset");
  }
...
Render3DError
OpenGLRenderer_1_2::CreateClearImageProgram(const char *vsCString,
                                            const char *fsCString) {
  Render3DError error = OGLERROR_NOERR;
  // TODO: Add support for ancient GPUs that support shaders but not
...
Render3DError
OpenGLRenderer_1_2::CreateEdgeMarkProgram(const bool isMultisample,
                                          const char *vtxShaderCString,
                                          const char *fragShaderCString) {
  Render3DError error = OGLERROR_NOERR;
  OGLRenderRef &OGLRef = *this->ref;
  ...
  const GLint uniformTexEdgeColorTable =
      glGetUniformLocation(OGLRef.programEdgeMarkID, "texEdgeColor");
...
void OpenGLRenderer_1_2::DestroyEdgeMarkProgram() {
  ...
  if (OGLRef.programMSEdgeMarkID != 0) {
    ...
    glDetachShader(OGLRef.programMSEdgeMarkID,
...
Render3DError OpenGLRenderer_1_2::CreateFogProgram(
    const OGLFogProgramKey fogProgramKey, const bool isMultisample,
    const char *vtxShaderCString, const char *fragShaderCString) {
  Render3DError error = OGLERROR_NOERR;
  OGLRenderRef &OGLRef = *this->ref;
  ...
  fragDepthConstants << "#define FOG_OFFSETF " << fogOffsetf
                     << (((fogOffsetf == 0.0f) || (fogOffsetf == 1.0f)) ? ".0"
                                                                        : "")
                     << "\n";
  ...
  error = ShaderProgramCreateOGL(OGLRef.vertexFogShaderID, shaderID.fragShader,
                                 shaderID.program, vtxShaderCString,
  ...
  const GLint uniformTexFogDensityTable =
      glGetUniformLocation(shaderID.program, "texFogDensityTable");
  glUniform1i(uniformTexGDepth, OGLTextureUnitID_DepthStencil);
...
Render3DError OpenGLRenderer_1_2::InitPostprocessingPrograms(
  ...
  if (this->_feature.supportVBO && this->_feature.supportFBO) {
    error = this->CreateEdgeMarkProgram(false, edgeMarkVtxShaderCString,
...
Render3DError OpenGLRenderer_1_2::UploadClearImage(
  ...
  const bool didColorChange =
      (memcmp(OGLRef.workingCIColorBuffer16, colorBuffer,
              GPU_FRAMEBUFFER_NATIVE_WIDTH * GPU_FRAMEBUFFER_NATIVE_HEIGHT *
                  sizeof(u16)) != 0);
  ...
  if (didColorChange) {
    memcpy(OGLRef.workingCIColorBuffer16, colorBuffer,
           GPU_FRAMEBUFFER_NATIVE_WIDTH * GPU_FRAMEBUFFER_NATIVE_HEIGHT *
               sizeof(u16));
    glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_CIColor);
    glBindTexture(GL_TEXTURE_2D, OGLRef.texCIColorID);
    ...
    if (OGLRef.textureSrcTypeCIColor == GL_UNSIGNED_BYTE) {
      ColorspaceConvertBuffer5551To8888<false, false, BESwapDst>(
          OGLRef.workingCIColorBuffer16, OGLRef.workingCIColorBuffer32,
          GPU_FRAMEBUFFER_NATIVE_WIDTH * GPU_FRAMEBUFFER_NATIVE_HEIGHT);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GPU_FRAMEBUFFER_NATIVE_WIDTH,
                      GPU_FRAMEBUFFER_NATIVE_HEIGHT, GL_RGBA,
                      OGLRef.textureSrcTypeCIColor,
                      OGLRef.workingCIColorBuffer32);
    } else {
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, GPU_FRAMEBUFFER_NATIVE_WIDTH,
                      GPU_FRAMEBUFFER_NATIVE_HEIGHT, GL_RGBA,
                      OGLRef.textureSrcTypeCIColor,
                      OGLRef.workingCIColorBuffer16);
    }
  }
  ...
  if (didFogAttributesChange) {
    ...
    glTexSubImage2D(
        ...
        GPU_FRAMEBUFFER_NATIVE_HEIGHT, GL_RGBA, OGLRef.textureSrcTypeCIFog,
        OGLRef.workingCIFogAttributesBuffer[this->_clearImageIndex]);
  }
...
Render3DError OpenGLRenderer_1_2::ZeroDstAlphaPass(
    const POLY *rawPolyList, const CPoly *clippedPolyList,
    const size_t clippedPolyCount, const size_t clippedPolyOpaqueCount,
  ...
  if (isRunningMSAA) {
    ...
    glBlitFramebufferEXT(
        0, 0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
        0, 0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
  ...
  FramebufferProcessVertexAttribEnableLEGACYOGL(this->_feature,
                                                OGLRef.vaoPostprocessStatesID,
                                                OGLRef.vboPostprocessVtxID);
  glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
  ...
  this->DrawPolygonsForIndexRange<OGLPolyDrawMode_ZeroAlphaPass>(
      rawPolyList, clippedPolyList, clippedPolyCount, clippedPolyOpaqueCount,
  ...
  this->_SetupGeometryShaders(this->_geometryProgramFlags);
  glDrawBuffers(4, this->_geometryDrawBuffersEnum[this->_geometryProgramFlags
                                                      .DrawBuffersMode]);
...
void OpenGLRenderer_1_2::_ResolveWorkingBackFacing() {
  ...
  glDrawBuffer(OGL_WORKING_ATTACHMENT_ID);
  glBlitFramebufferEXT(
      0, 0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight, 0,
      0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
      GL_COLOR_BUFFER_BIT, GL_NEAREST);
  glReadBuffer(OGL_COLOROUT_ATTACHMENT_ID);
...
Render3DError
OpenGLRenderer_1_2::BeginRender(const GFX3D_State &renderState,
  ...
  if (this->_clippedPolyCount > 0) {
    if (this->_feature.supportVBO) {
      ...
      if (this->_feature.supportShaders) {
        ...
        glBufferSubData(GL_ARRAY_BUFFER, 0,
                        sizeof(NDSVertex) * renderGList.rawVertCount,
                        renderGList.rawVtxList);
      } else {
        // If shaders aren't supported, we need to use the client-side buffers
        // here.
        glBindBuffer(GL_ARRAY_BUFFER, 0);
      }
    }
    ...
    for (size_t i = 0, vertIndexCount = 0; i < this->_clippedPolyCount; i++) {
      ...
      if (this->_feature.supportShaders) {
        ...
        const GLfloat thePolyAlpha =
            (GFX3D_IsPolyWireframe(rawPoly))
                ? 1.0f
                : divide5bitBy31_LUT[rawPoly.attribute.Alpha];
        ...
        for (size_t j = 0; j < polyType; j++) {
          ...
          // Since we can't use shaders, we can't perform any data format
          // conversions of the vertex data on the GPU. Therefore, we need to do
          // the conversions on the CPU instead.
          OGLRef.position4fBuffer[positionIndex + 0] =
              (float)vtx.position.x / 4096.0f;
          OGLRef.position4fBuffer[positionIndex + 1] =
              (float)vtx.position.y / 4096.0f;
          OGLRef.position4fBuffer[positionIndex + 2] =
              (float)vtx.position.z / 4096.0f;
          OGLRef.position4fBuffer[positionIndex + 3] =
              (float)vtx.position.w / 4096.0f;

          OGLRef.texCoord2fBuffer[texCoordIndex + 0] =
              (float)vtx.texCoord.s / 16.0f;
          OGLRef.texCoord2fBuffer[texCoordIndex + 1] =
              (float)vtx.texCoord.t / 16.0f;

          OGLRef.color4fBuffer[colorIndex + 0] =
              divide6bitBy63_LUT[vtx.color.r];
          OGLRef.color4fBuffer[colorIndex + 1] =
              divide6bitBy63_LUT[vtx.color.g];
          OGLRef.color4fBuffer[colorIndex + 2] =
              divide6bitBy63_LUT[vtx.color.b];
    ...
    this->_pendingRenderStates.enableAntialiasing =
        (renderState.DISP3DCNT.EnableAntialiasing) ? GL_TRUE : GL_FALSE;
    this->_pendingRenderStates.enableFogAlphaOnly =
        (renderState.DISP3DCNT.FogOnlyAlpha) ? GL_TRUE : GL_FALSE;
    this->_pendingRenderStates.clearPolyID =
        this->_clearAttributes.opaquePolyID;
    this->_pendingRenderStates.clearDepth =
        (GLfloat)this->_clearAttributes.depth / (GLfloat)0x00FFFFFF;
    this->_pendingRenderStates.alphaTestRef =
        divide5bitBy31_LUT[renderState.alphaTestRef];
    ...
    if (this->_enableFog && this->_deviceInfo.isFogSupported) {
      ...
      this->_pendingRenderStates.fogColor.r =
          divide5bitBy31_LUT[(renderState.fogColor) & 0x0000001F];
      this->_pendingRenderStates.fogColor.g =
          divide5bitBy31_LUT[(renderState.fogColor >> 5) & 0x0000001F];
      this->_pendingRenderStates.fogColor.b =
          divide5bitBy31_LUT[(renderState.fogColor >> 10) & 0x0000001F];
      this->_pendingRenderStates.fogColor.a =
          divide5bitBy31_LUT[(renderState.fogColor >> 16) & 0x0000001F];
      this->_pendingRenderStates.fogOffset =
          (GLfloat)(renderState.fogOffset & 0x7FFF) / 32767.0f;
      this->_pendingRenderStates.fogStep =
          (GLfloat)(0x0400 >> renderState.fogShift) / 32767.0f;

      u8 fogDensityTable[32];
      for (size_t i = 0; i < 32; i++) {
        fogDensityTable[i] = (renderState.fogDensityTable[i] == 127)
                                 ? 255
                                 : renderState.fogDensityTable[i] << 1;
      }

      glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_LookupTable);
      glBindTexture(GL_TEXTURE_1D, OGLRef.texFogDensityTableID);
      glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 32, GL_RED, GL_UNSIGNED_BYTE,
                      fogDensityTable);
    }

    if (this->_enableEdgeMark && this->_deviceInfo.isEdgeMarkSupported) {
      const u8 alpha8 =
          (renderState.DISP3DCNT.EnableAntialiasing) ? 0x80 : 0xFF;
      Color4u8 edgeColor32[8];

      for (size_t i = 0; i < 8; i++) {
        edgeColor32[i].value =
            COLOR555TO8888(renderState.edgeMarkColorTable[i] & 0x7FFF, alpha8);
      }

      glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_LookupTable);
      glBindTexture(GL_TEXTURE_1D, OGLRef.texEdgeColorTableID);
      glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 8, GL_RGBA,
                      OGLRef.textureSrcTypeEdgeColor, edgeColor32);
    }

    if (this->_feature.supportShaders) {
      this->_geometryProgramFlags.value = 0;
      this->_geometryProgramFlags.EnableWDepth =
          renderState.SWAP_BUFFERS.DepthMode;
      ...
      this->_geometryProgramFlags.ToonShadingMode =
          renderState.DISP3DCNT.PolygonShading;
      this->_geometryProgramFlags.EnableFog =
          (this->_enableFog && this->_deviceInfo.isFogSupported) ? 1 : 0;
      this->_geometryProgramFlags.EnableEdgeMark =
          (this->_enableEdgeMark && this->_deviceInfo.isEdgeMarkSupported) ? 1
                                                                           : 0;
      ...
      if (renderNeedsToonTable) {
        ...
        if (OGLRef.textureSrcTypeToonTable == GL_UNSIGNED_BYTE) {
          ColorspaceConvertBuffer555xTo8888Opaque<false, false, BESwapDst>(
              renderState.toonTable16, OGLRef.toonTable32, 32);
          glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 32, GL_RGBA,
                          OGLRef.textureSrcTypeToonTable, OGLRef.toonTable32);
        } else {
          glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 32, GL_RGBA,
                          OGLRef.textureSrcTypeToonTable,
                          renderState.toonTable16);
        }
      }
    }
#if defined(GL_VERSION_1_2)
    else {
      if (renderState.DISP3DCNT.EnableAlphaTest &&
          (renderState.alphaTestRef > 0)) {
    ...
    // Even with no polygons to draw, we always need to set these 3 flags so
    // that glDrawBuffers() can reference the correct set of FBO attachments
    // using OGLGeometryFlags.DrawBuffersMode.
    this->_geometryProgramFlags.EnableFog =
        (this->_enableFog && this->_deviceInfo.isFogSupported) ? 1 : 0;
    this->_geometryProgramFlags.EnableEdgeMark =
        (this->_enableEdgeMark && this->_deviceInfo.isEdgeMarkSupported) ? 1
...
void OpenGLRenderer_1_2::_RenderGeometryLoopBegin() {
  ...
  glDisable(
      GL_CULL_FACE); // Polygons should already be culled before we get here.
  glEnable(GL_DEPTH_TEST);
  glEnable(GL_STENCIL_TEST);
  ...
  if (this->_feature.supportFBO) {
    ...
    if (this->_feature.supportShaders) {
      glDrawBuffers(4,
                    this->_geometryDrawBuffersEnum[this->_geometryProgramFlags
                                                       .DrawBuffersMode]);
    } else {
  ...
  if (this->_clippedPolyCount > 0) {
    ...
    if (this->_clippedPolyOpaqueCount > 0) {
      ...
      this->DrawPolygonsForIndexRange<OGLPolyDrawMode_DrawOpaquePolys>(
          rawPolyList, this->_clippedPolyList, this->_clippedPolyCount, 0,
    ...
    if (this->_clippedPolyOpaqueCount < this->_clippedPolyCount) {
      ...
      // Geometry flags have changed, so we need to update shader uniforms and
      // also update draw buffers based on the flags.
      this->_SetupGeometryShaders(this->_geometryProgramFlags);
      ...
      if (this->_needsZeroDstAlphaPass &&
        ...
        if (this->_clippedPolyOpaqueCount == 0) {
          this->SetupPolygon(firstPoly, true, false,
                             firstCPoly.isPolyBackFacing);
        }

        this->ZeroDstAlphaPass(
            rawPolyList, this->_clippedPolyList, this->_clippedPolyCount,
            this->_clippedPolyOpaqueCount, this->_enableAlphaBlending,
            indexOffset, lastPolyAttr);

        if (this->_clippedPolyOpaqueCount > 0) {
          const CPoly &lastOpaqueCPoly =
              this->_clippedPolyList[this->_clippedPolyOpaqueCount - 1];
          ...
          this->SetupPolygon(lastOpaquePoly, false, true,
                             lastOpaqueCPoly.isPolyBackFacing);
        }
      } else {
  ...
  if (this->_enableFog && this->_deviceInfo.isFogSupported) {
    ...
    if (it == this->_fogProgramMap.end()) {
      Render3DError error = this->CreateFogProgram(
          this->_fogProgramKey, false, FogVtxShader_100, FogFragShader_100);
      if (error != OGLERROR_NOERR) {
        return error;
      }
    }
    ...
    glBlendFuncSeparate(GL_CONSTANT_COLOR, GL_ONE_MINUS_SRC_COLOR,
...
Render3DError OpenGLRenderer_1_2::ClearUsingImage(
  ...
  if (this->_feature.supportShaders) {
    ...
    if (this->_enableFog && this->_deviceInfo.isFogSupported) {
      ...
      glDrawBuffer(OGL_FOGATTRIBUTES_ATTACHMENT_ID);
      glBlitFramebufferEXT(
          0, 0, GPU_FRAMEBUFFER_NATIVE_WIDTH, GPU_FRAMEBUFFER_NATIVE_HEIGHT, 0,
          0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
          GL_COLOR_BUFFER_BIT, GL_NEAREST);
    }
    ...
    glDrawBuffer(OGL_COLOROUT_ATTACHMENT_ID);
    glBlitFramebufferEXT(
        0, 0, GPU_FRAMEBUFFER_NATIVE_WIDTH, GPU_FRAMEBUFFER_NATIVE_HEIGHT, 0, 0,
        (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
        GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT, GL_NEAREST);
  } else {
    glBlitFramebufferEXT(
        0, 0, GPU_FRAMEBUFFER_NATIVE_WIDTH, GPU_FRAMEBUFFER_NATIVE_HEIGHT, 0, 0,
  ...
  if (this->_enableMultisampledRendering) {
    ...
    // See above comment for why we need to get clear the stencil buffer
    // separately.
    glClearStencil(opaquePolyID);
    glClear(GL_STENCIL_BUFFER_BIT);
    ...
    if (this->_feature.supportShaders) {
      ...
      if (this->_enableFog && this->_deviceInfo.isFogSupported) {
        ...
        glBlitFramebufferEXT(0, 0, (GLint)this->_framebufferWidth,
                             (GLint)this->_framebufferHeight, 0, 0,
                             (GLint)this->_framebufferWidth,
                             (GLint)this->_framebufferHeight,
                             GL_COLOR_BUFFER_BIT, GL_NEAREST);
      }

      // Blit the color and depth buffers. Do this last so that color attachment
      // 0 is set to the read FBO.
      glReadBuffer(OGL_COLOROUT_ATTACHMENT_ID);
      glDrawBuffer(OGL_COLOROUT_ATTACHMENT_ID);
      glBlitFramebufferEXT(
          0, 0, (GLint)this->_framebufferWidth, (GLint)this->_framebufferHeight,
...
Render3DError OpenGLRenderer_1_2::ClearUsingValues(
    const Color4u8 &clearColor6665, const FragmentAttributes &clearAttributes) {
  OGLRenderRef &OGLRef = *this->ref;
...
Render3DError OpenGLRenderer_1_2::SetupPolygon(const POLY &thePoly,
  ...
  if (this->_feature.supportShaders) {
    ...
    glUniform1i(
        ...
        (GFX3D_IsPolyWireframe(thePoly)) ? GL_TRUE : GL_FALSE);
    glUniform1i(
    ...
    glUniform1f(
        OGLRef.uniformPolyDepthOffset[this->_geometryProgramFlags.value], 0.0f);
    glUniform1i(
        OGLRef.uniformPolyIsBackFacing[this->_geometryProgramFlags.value],
        (isBackFacing) ? GL_TRUE : GL_FALSE);
  }
#if defined(GL_VERSION_1_2)
  else {
...
Render3DError OpenGLRenderer_1_2::SetupViewport(const GFX3D_Viewport viewport) {
  ...
  glViewport(viewport.x * wScalar,
             (-viewport.y + (192 - viewport.height)) *
                 hScalar, // All vertex Y-coordinates will be flipped, so the
                          // viewport Y positioning needs to be flipped as well.
...
Render3DError OpenGLRenderer_1_2::RenderFlush(bool willFlushBuffer32,
  ...
  Render3DError error =
      this->Render3D::RenderFlush(willFlushBuffer32, willFlushBuffer16);
...
Render3DError OpenGLRenderer_1_2::SetFramebufferSize(size_t w, size_t h) {
  ...
  if (this->_feature.supportFBO) {
    glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_DepthStencil);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8_EXT, (GLsizei)w,
                 (GLsizei)h, 0, GL_DEPTH_STENCIL_EXT, GL_UNSIGNED_INT_24_8_EXT,
                 NULL);
  ...
  if (this->_feature.supportShaders) {
    ...
    if (this->_feature.supportVBO && this->_feature.supportFBO) {
      this->CreateEdgeMarkProgram(false, EdgeMarkVtxShader_100,
                                  EdgeMarkFragShader_100);
    }
  }
  ...
  if (oglrender_framebufferDidResizeCallback != NULL) {
    bool clientResizeSuccess =
        oglrender_framebufferDidResizeCallback(this->_feature.supportFBO, w, h);
...
Render3DError
OpenGLRenderer_2_0::BeginRender(const GFX3D_State &renderState,
                                const GFX3D_GeometryList &renderGList) {
  OGLRenderRef &OGLRef = *this->ref;
