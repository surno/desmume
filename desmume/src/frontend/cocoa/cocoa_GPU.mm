...
- (id)init {
  ...
  _gpuScale = 1;
  gpuStateFlags =
      GPUSTATE_MAIN_GPU_MASK | GPUSTATE_MAIN_BG0_MASK | GPUSTATE_MAIN_BG1_MASK |
      GPUSTATE_MAIN_BG2_MASK | GPUSTATE_MAIN_BG3_MASK | GPUSTATE_MAIN_OBJ_MASK |
      GPUSTATE_SUB_GPU_MASK | GPUSTATE_SUB_BG0_MASK | GPUSTATE_SUB_BG1_MASK |
      GPUSTATE_SUB_BG2_MASK | GPUSTATE_SUB_BG3_MASK | GPUSTATE_SUB_OBJ_MASK;
...
- (void)dealloc {
  GPU->SetEventHandler(
      NULL); // Unassigned our event handler before we delete it.
...
- (void)setGpuScale:(NSUInteger)theScale {
  ...
  [self setGpuDimensions:NSMakeSize(GPU_FRAMEBUFFER_NATIVE_WIDTH * theScale,
                                    GPU_FRAMEBUFFER_NATIVE_HEIGHT * theScale)];
}
...
- (void)setRender3DRenderingEngine:(NSInteger)rendererID {
  if (rendererID < CORE3DLIST_NULL) {
    ...
    puts(
        "DeSmuME: Invalid 3D renderer chosen; falling back to SoftRasterizer.");
...
- (NSString *)render3DRenderingEngineAppliedHostRendererName {
  ...
  theString = [NSString stringWithCString:theName.c_str()
...
- (BOOL)render3DHighPrecisionColorInterpolation {
  gpuEvent->ApplyRender3DSettingsLock();
  const BOOL state =
      CommonSettings.GFX3D_HighResolutionInterpolateColor ? YES : NO;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (void)setRender3DThreads:(NSUInteger)numberThreads {
  ...
  if (numberThreads == 0) {
    ...
    if (numberCores < 2) {
      ...
      const int reserveCoreCount =
          numberCores /
          12; // For every 12 cores, reserve 1 core for the rest of the system.
  ...
  const RendererID renderingEngineID =
      (RendererID)[self render3DRenderingEngine];
...
- (void)setRender3DMultisampleSize:(NSUInteger)msaaSize {
  ...
  const NSUInteger currentMSAASize =
      (NSUInteger)CommonSettings.GFX3D_Renderer_MultisampleSize;
  ...
  NSString *newMsaaSizeString =
      (msaaSize == 0) ? @"Off"
                      : [NSString stringWithFormat:@"%d", (int)msaaSize];
  [self setRender3DMultisampleSizeString:newMsaaSizeString];
}

- (NSUInteger)render3DMultisampleSize {
  gpuEvent->ApplyRender3DSettingsLock();
  const NSInteger msaaSize =
      (NSUInteger)CommonSettings.GFX3D_Renderer_MultisampleSize;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (BOOL)render3DTextureDeposterize {
  gpuEvent->ApplyRender3DSettingsLock();
  const BOOL state =
      CommonSettings.GFX3D_Renderer_TextureDeposterize ? YES : NO;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (void)setRender3DTextureScalingFactor:(NSUInteger)scalingFactor {
  ...
  if (newScalingFactor == 3) {
    newScalingFactor =
        (newScalingFactor < CommonSettings.GFX3D_Renderer_TextureScalingFactor)
            ? 2
            : 4;
  }
...
- (NSUInteger)render3DTextureScalingFactor {
  gpuEvent->ApplyRender3DSettingsLock();
  const NSUInteger scalingFactor =
      (NSUInteger)CommonSettings.GFX3D_Renderer_TextureScalingFactor;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (BOOL)openGLEmulateSpecialZeroAlphaBlending {
  gpuEvent->ApplyRender3DSettingsLock();
  const BOOL state =
      (CommonSettings.OpenGL_Emulation_SpecialZeroAlphaBlending) ? YES : NO;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (BOOL)openGLEmulateNDSDepthCalculation {
  gpuEvent->ApplyRender3DSettingsLock();
  const BOOL state =
      (CommonSettings.OpenGL_Emulation_NDSDepthCalculation) ? YES : NO;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (BOOL)openGLEmulateDepthLEqualPolygonFacing {
  gpuEvent->ApplyRender3DSettingsLock();
  const BOOL state =
      (CommonSettings.OpenGL_Emulation_DepthLEqualPolygonFacing) ? YES : NO;
  gpuEvent->ApplyRender3DSettingsUnlock();
...
- (void)setLayerMainGPU:(BOOL)gpuState {
  ...
  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (gpuState) ? (gpuStateFlags | GPUSTATE_MAIN_GPU_MASK)
...
- (void)setLayerMainBG0:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineMain()->SetLayerEnableState(GPULayerID_BG0,
                                            (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_MAIN_BG0_MASK)
...
- (BOOL)layerMainBG0 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineMain()->GetLayerEnableState(GPULayerID_BG0);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerMainBG1:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineMain()->SetLayerEnableState(GPULayerID_BG1,
                                            (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_MAIN_BG1_MASK)
                               : (gpuStateFlags & ~GPUSTATE_MAIN_BG1_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerMainBG1 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineMain()->GetLayerEnableState(GPULayerID_BG1);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerMainBG2:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineMain()->SetLayerEnableState(GPULayerID_BG2,
                                            (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_MAIN_BG2_MASK)
                               : (gpuStateFlags & ~GPUSTATE_MAIN_BG2_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerMainBG2 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineMain()->GetLayerEnableState(GPULayerID_BG2);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerMainBG3:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineMain()->SetLayerEnableState(GPULayerID_BG3,
                                            (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_MAIN_BG3_MASK)
                               : (gpuStateFlags & ~GPUSTATE_MAIN_BG3_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerMainBG3 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineMain()->GetLayerEnableState(GPULayerID_BG3);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerMainOBJ:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineMain()->SetLayerEnableState(GPULayerID_OBJ,
                                            (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_MAIN_OBJ_MASK)
                               : (gpuStateFlags & ~GPUSTATE_MAIN_OBJ_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerMainOBJ {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineMain()->GetLayerEnableState(GPULayerID_OBJ);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerSubGPU:(BOOL)gpuState {
  ...
  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (gpuState) ? (gpuStateFlags | GPUSTATE_SUB_GPU_MASK)
                             : (gpuStateFlags & ~GPUSTATE_SUB_GPU_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (void)setLayerSubBG0:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineSub()->SetLayerEnableState(GPULayerID_BG0,
                                           (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_SUB_BG0_MASK)
...
- (BOOL)layerSubBG0 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineSub()->GetLayerEnableState(GPULayerID_BG0);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerSubBG1:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineSub()->SetLayerEnableState(GPULayerID_BG1,
                                           (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_SUB_BG1_MASK)
                               : (gpuStateFlags & ~GPUSTATE_SUB_BG1_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerSubBG1 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineSub()->GetLayerEnableState(GPULayerID_BG1);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerSubBG2:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineSub()->SetLayerEnableState(GPULayerID_BG2,
                                           (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_SUB_BG2_MASK)
                               : (gpuStateFlags & ~GPUSTATE_SUB_BG2_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerSubBG2 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineSub()->GetLayerEnableState(GPULayerID_BG2);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerSubBG3:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineSub()->SetLayerEnableState(GPULayerID_BG3,
                                           (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_SUB_BG3_MASK)
                               : (gpuStateFlags & ~GPUSTATE_SUB_BG3_MASK);
  apple_unfairlock_unlock(_unfairlockGpuState);
}
...
- (BOOL)layerSubBG3 {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineSub()->GetLayerEnableState(GPULayerID_BG3);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)setLayerSubOBJ:(BOOL)layerState {
  gpuEvent->ApplyGPUSettingsLock();
  GPU->GetEngineSub()->SetLayerEnableState(GPULayerID_OBJ,
                                           (layerState) ? true : false);
  gpuEvent->ApplyGPUSettingsUnlock();

  apple_unfairlock_lock(_unfairlockGpuState);
  gpuStateFlags = (layerState) ? (gpuStateFlags | GPUSTATE_SUB_OBJ_MASK)
...
- (BOOL)layerSubOBJ {
  gpuEvent->ApplyGPUSettingsLock();
  const BOOL layerState =
      GPU->GetEngineSub()->GetLayerEnableState(GPULayerID_OBJ);
  gpuEvent->ApplyGPUSettingsUnlock();
...
- (void)clearWithColor:(const uint16_t)colorBGRA5551 {
  ...
  for (size_t i = 0; i < maxPages; i++) {
    semaphore_wait(((MacGPUFetchObjectAsync *)fetchObject)
                       ->SemaphoreFramebufferPageAtIndex(i));
  }
#endif
  ...
  for (size_t i = maxPages - 1; i < maxPages; i--) {
    semaphore_signal(((MacGPUFetchObjectAsync *)fetchObject)
                         ->SemaphoreFramebufferPageAtIndex(i));
  }

  const u8 bufferIndex = GPU->GetDisplayInfo().bufferIndex;
  ((MacGPUFetchObjectAsync *)fetchObject)
      ->SignalFetchAtIndex(bufferIndex, MESSAGE_FETCH_AND_PERFORM_ACTIONS);
#endif
}
...
bool MacGPUFetchObjectAsync::GetPauseState() { return this->_pauseState; }

void MacGPUFetchObjectAsync::SemaphoreFramebufferCreate() {
  this->_taskEmulationLoop = mach_task_self();

  for (size_t i = 0; i < MAX_FRAMEBUFFER_PAGES; i++) {
    semaphore_create(this->_taskEmulationLoop, &this->_semFramebuffer[i],
                     SYNC_POLICY_FIFO, 1);
  }
}
...
uint8_t MacGPUFetchObjectAsync::SelectBufferIndex(const uint8_t currentIndex,
                                                  size_t pageCount) {
  ...
  if (stillSearching) {
    ...
    for (; selectedIndex != currentIndex;
         selectedIndex = (selectedIndex + 1) % pageCount) {
      if (this->FramebufferStateAtIndex(selectedIndex) ==
          ClientDisplayBufferState_Idle) {
  ...
  // Next, search for either an idle or a ready buffer along with its
  // corresponding semaphore.
  if (stillSearching) {
    selectedIndex = (selectedIndex + 1) % pageCount;
    for (size_t spin = 0; spin < 100ULL * pageCount;
         selectedIndex = (selectedIndex + 1) % pageCount, spin++) {
      if ((this->FramebufferStateAtIndex(selectedIndex) ==
           ClientDisplayBufferState_Idle) ||
          ((this->FramebufferStateAtIndex(selectedIndex) ==
  ...
  // Since the most available buffers couldn't be taken, we're going to spin for
  // some finite period of time until an idle buffer emerges. If that happens,
  // then force wait on the buffer's corresponding semaphore.
  if (stillSearching) {
    selectedIndex = (selectedIndex + 1) % pageCount;
    for (size_t spin = 0; spin < 10000ULL * pageCount;
         selectedIndex = (selectedIndex + 1) % pageCount, spin++) {
      if (this->FramebufferStateAtIndex(selectedIndex) ==
          ClientDisplayBufferState_Idle) {
        stillSearching = false;
  ...
  // In an effort to find something that is likely to be available shortly in
  // the future, search for any idle, ready or reading buffer, and then force
  // wait on its corresponding semaphore.
  if (stillSearching) {
    selectedIndex = (selectedIndex + 1) % pageCount;
    for (; selectedIndex != currentIndex;
         selectedIndex = (selectedIndex + 1) % pageCount) {
      if ((this->FramebufferStateAtIndex(selectedIndex) ==
          ...
          (this->FramebufferStateAtIndex(selectedIndex) ==
           ClientDisplayBufferState_Ready) ||
          (this->FramebufferStateAtIndex(selectedIndex) ==
           ClientDisplayBufferState_Reading)) {
        stillSearching = false;
        break;
      }
    }
...
semaphore_t
MacGPUFetchObjectAsync::SemaphoreFramebufferPageAtIndex(const u8 bufferIndex) {
  assert(bufferIndex < MAX_FRAMEBUFFER_PAGES);
  return this->_semFramebuffer[bufferIndex];
}

ClientDisplayBufferState
MacGPUFetchObjectAsync::FramebufferStateAtIndex(uint8_t index) {
...
void MacGPUFetchObjectAsync::SetFramebufferState(
    ClientDisplayBufferState bufferState, uint8_t index) {
  apple_unfairlock_lock(this->_unfairlockFramebufferStates[index]);
...
void MacGPUFetchObjectAsync::SignalFetchAtIndex(uint8_t index,
                                                int32_t messageID) {
...
static void ScreenChangeCallback(CFNotificationCenterRef center, void *observer,
...
static CVReturn MacDisplayLinkCallback(CVDisplayLinkRef displayLinkRef,
  ...
  MacGPUFetchObjectDisplayLink *fetchObj =
      (MacGPUFetchObjectDisplayLink *)displayLinkContext;
...
MacGPUFetchObjectDisplayLink::MacGPUFetchObjectDisplayLink() {
  ...
  CFNotificationCenterAddObserver(
      CFNotificationCenterGetLocalCenter(), this, ScreenChangeCallback,
      CFSTR("NSApplicationDidChangeScreenParametersNotification"), NULL,
...
void MacGPUFetchObjectDisplayLink::DisplayLinkStartUsingID(
    CGDirectDisplayID displayID) {
  CVDisplayLinkRef displayLinkRef = NULL;

  pthread_mutex_lock(&this->_mutexDisplayLinkLists);

  if (this->_displayLinksActiveList.find(displayID) !=
      this->_displayLinksActiveList.end()) {
...
void MacGPUFetchObjectDisplayLink::DisplayLinkListUpdate() {
  ...
  for (size_t i = 0; i < [screenList count]; i++) {
    ...
    NSNumber *idNumber =
        (NSNumber *)[deviceDescription valueForKey:@"NSScreenNumber"];

    CGDirectDisplayID displayID = [idNumber unsignedIntValue];
    bool isDisplayLinkStillActive =
        (this->_displayLinksActiveList.find(displayID) !=
         this->_displayLinksActiveList.end());

    if (!isDisplayLinkStillActive) {
      CVDisplayLinkRef newDisplayLink;
      CVDisplayLinkCreateWithCGDisplay(displayID, &newDisplayLink);
      CVDisplayLinkSetOutputCallback(newDisplayLink, &MacDisplayLinkCallback,
                                     this);
    ...
    // While we're iterating through NSScreens, save the CGDirectDisplayID to a
  ...
  for (DisplayLinksActiveMap::iterator it =
           this->_displayLinksActiveList.begin();
       it != this->_displayLinksActiveList.end();) {
    CGDirectDisplayID displayID = it->first;
    CVDisplayLinkRef displayLinkRef = it->second;

    if (screenActiveDisplayIDsList.find(displayID) ==
        screenActiveDisplayIDsList.end()) {
      if (CVDisplayLinkIsRunning(displayLinkRef)) {
        CVDisplayLinkStop(displayLinkRef);
        this->_displayLinksStartedList.erase(displayID);
      }
...
void MacGPUFetchObjectDisplayLink::FlushAllViewsOnDisplayLink(
    CVDisplayLinkRef displayLinkRef, const CVTimeStamp *timeStampNow,
    const CVTimeStamp *timeStampOutput) {
  CGDirectDisplayID displayID =
      CVDisplayLinkGetCurrentCGDisplay(displayLinkRef);
  bool didFlushOccur = false;

  std::vector<ClientDisplayViewInterface *> cdvFlushList;
  this->_outputManager->GenerateFlushListForDisplay((int32_t)displayID,
                                                    cdvFlushList);

  const size_t listSize = cdvFlushList.size();
  if (listSize > 0) {
    this->FlushMultipleViews(cdvFlushList, (uint64_t)timeStampNow->videoTime,
                             timeStampOutput->hostTime);
    didFlushOccur = true;
  }

  if (didFlushOccur) {
    // Set the new time limit to 8 seconds after the current time.
    this->_displayLinkFlushTimeList[displayID] =
        timeStampNow->videoTime +
        (timeStampNow->videoTimeScale * VIDEO_FLUSH_TIME_LIMIT_OFFSET);
  } else if (timeStampNow->videoTime >
             this->_displayLinkFlushTimeList[displayID]) {
    CVDisplayLinkStop(displayLinkRef);
    this->_displayLinksStartedList.erase(displayID);
  }
...
void MacGPUFetchObjectDisplayLink::SetPauseState(bool theState) {
  ...
  if (theState) {
    for (DisplayLinksActiveMap::iterator it =
             this->_displayLinksStartedList.begin();
         it != this->_displayLinksStartedList.end(); it++) {
      CVDisplayLinkRef displayLinkRef = it->second;
      if ((displayLinkRef != NULL) && CVDisplayLinkIsRunning(displayLinkRef)) {
        CVDisplayLinkStop(displayLinkRef);
      }
...
void MacGPUEventHandlerAsync::SetFetchObject(
...
void MacGPUEventHandlerAsync::GetFramebufferDimensions(size_t &outWidth,
                                                       size_t &outHeight) {
...
void MacGPUEventHandlerAsync::DidFrameBegin(const size_t line,
                                            const bool isFrameSkipRequested,
                                            const size_t pageCount,
                                            u8 &selectedBufferIndexInOut) {
  MacGPUFetchObjectAsync *asyncFetchObj =
      (MacGPUFetchObjectAsync *)this->_fetchObject;

  if (!isFrameSkipRequested) {
    if ((pageCount > 1) && (line == 0)) {
      selectedBufferIndexInOut =
          asyncFetchObj->SelectBufferIndex(selectedBufferIndexInOut, pageCount);
    }

    semaphore_wait(asyncFetchObj->SemaphoreFramebufferPageAtIndex(
        selectedBufferIndexInOut));
    asyncFetchObj->SetFramebufferState(ClientDisplayBufferState_Writing,
                                       selectedBufferIndexInOut);
  }
}

void MacGPUEventHandlerAsync::DidFrameEnd(
    bool isFrameSkipped, const NDSDisplayInfo &latestDisplayInfo) {
  MacGPUFetchObjectAsync *asyncFetchObj =
      (MacGPUFetchObjectAsync *)this->_fetchObject;
  ...
  if (!isFrameSkipped) {
    ...
    asyncFetchObj->SetFramebufferState(ClientDisplayBufferState_Ready,
                                       latestDisplayInfo.bufferIndex);
    semaphore_signal(asyncFetchObj->SemaphoreFramebufferPageAtIndex(
        latestDisplayInfo.bufferIndex));
    asyncFetchObj->SignalFetchAtIndex(latestDisplayInfo.bufferIndex,
...
void MacGPUEventHandlerAsync::DidApplyGPUSettingsBegin() {
  ...
  if (this->_didWidthChange || this->_didHeightChange ||
      this->_didColorFormatChange || this->_didPageCountChange) {
    MacGPUFetchObjectAsync *asyncFetchObj =
        (MacGPUFetchObjectAsync *)this->_fetchObject;
...
void MacGPUEventHandlerAsync::DidApplyGPUSettingsEnd() {
  if (this->_didWidthChange || this->_didHeightChange ||
    ...
    this->_didColorFormatChange =
        (this->_colorFormatPending != displayInfo.colorFormat);
    this->_didPageCountChange =
        (this->_pageCountPending != displayInfo.framebufferPageCount);
  }
